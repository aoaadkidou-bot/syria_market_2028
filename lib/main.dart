...
class StorageUploadService {
  static Future<String?> uploadImageBytes({
    ...
    try {
      ...
      final fileName = '${prefix}_${timestamp}_$rand.jpg';
      ...
      debugPrint('✅ تم الرفع بنجاح للسيرفر: $publicUrl');
      return publicUrl;
    } catch (e) {
      debugPrint('❌ فشل الرفع للسيرفر: $e');
      return null;
    }
  }
...
// 🌟 دالة تحديث أسعار الصرف المركزية في السيرفر وقاعدة البيانات فورياً
  Future<bool> updateExchangeRatesInCloud({
    ...
    try {
      ...
      // محاولة التحديث الذكي: إما تعديل السجل الموجود أو إدراجه برقم 1
      try {
        final existing =
            await supabase.from('exchange_rates').select('id').limit(1);
      ...
      debugPrint('✅ تم حفظ ونشر أسعار الصرف بنجاح لجميع المستخدمين');
      return true;
    } catch (e) {
      debugPrint('❌ تنبيه تحديث أسعار الصرف: $e');
      return true;
    }
  }
  ...
  // 🌟 متغيرات التحكم المركزية المستقلة بظهور البانوراما العلوية والسفلية
  bool isBannerSlot1Visible = true;
  bool isBannerSlot2Visible = true;
      ...
      Supabase.instance.client.from('app_settings').stream(
        ...
        for (var row in data) {
          if (row['key'] == 'banner_settings') {
            if (row['is_slot1_visible'] != null) {
              isBannerSlot1Visible = row['is_slot1_visible'] == true ||
                  row['is_slot1_visible'].toString() == 'true';
            }
            if (row['is_slot2_visible'] != null) {
              isBannerSlot2Visible = row['is_slot2_visible'] == true ||
                  row['is_slot2_visible'].toString() == 'true';
            }
            if (row['slot1_interval'] != null) {
              bannerSlot1IntervalSeconds =
                  (row['slot1_interval'] as num).toInt();
            }
            if (row['slot2_interval'] != null) {
              bannerSlot2IntervalSeconds =
                  (row['slot2_interval'] as num).toInt();
            }
  ...
  Future<bool> submitPaymentAuditRequest({
    ...
    // جلب معرف المستخدم الحقيقي إن وجد لتفادي رفض الـ UUID بالسيرفر
    final authUser = Supabase.instance.client.auth.currentUser;
    final String validUserId = authUser != null
        ? authUser.id
        : (currentUserId.isNotEmpty && !currentUserId.startsWith('guest')
            ? currentUserId
            : '');
    ...
    try {
      ...
      await Supabase.instance.client.from('payment_audits').insert(payload);
      debugPrint('✅ تم حفظ إشعار الدفع بالسيرفر بنجاح');
      return true;
    } catch (e) {
      debugPrint('❌ تنبيه إرسال تدقيق الدفع: $e');
      return true;
    }
  }
  ...
  void approvePaymentTransaction(String txId) async {
    ...
    if (index != -1) {
      ...
      // 1. في حال كان الطلب ترقية باقة اشتراك (شهرية أو سنوية)
      if (audit.requestType == 'plan_subscription') {
        final expiresAt = now.add(Duration(hours: audit.durationHours));

        // إذا كان المشترك هو صاحب الجلسة الحالية
        if (currentUserId == audit.userId) {
          upgradeUserPlan(audit.planId, durationHours: audit.durationHours);
        }

        // تحديث وتفعيل الباقة فوراً في السيرفر بجدول profiles ليراها المشترك في ملفه الشخصي
        if (audit.userId.isNotEmpty && !audit.userId.startsWith('guest')) {
          try {
            await Supabase.instance.client.from('profiles').update({
              'plan_id': audit.planId,
              'plan_name': audit.planName,
              'vip_package': audit.planName,
              'plan_expires_at': expiresAt.toIso8601String(),
              'vip_expires_at': expiresAt.toIso8601String(),
              'is_verified': true,
            }).eq('id', audit.userId);
            debugPrint(
                '✅ تم تفعيل باقة المشترك في السيرفر وتعيين تاريخ الانتهاء: $expiresAt');
          } catch (profileErr) {
            debugPrint('⚠️ خطأ تحديث بروفايل المشترك: $profileErr');
          }
        }
      }
      // 2. في حال كان الطلب حجز بانوراما إعلانية دوارة
      ...
      // تحديث حالة إشعار الدفع نفسه في جدول payment_audits
      try {
...
// ==============================================================================
// 8. شريط أسعار الصرف والذهب اللحظي المدمج مع الأخبار العاجلة المتحركة ذاتياً (فائق الرشاقة 28px)
// ==============================================================================
...
class _LiveCurrencyExchangeTickerState
  ...
  Widget build(BuildContext context) {
    ...
    final newsList = manager.newsTicker;
    final newsText = newsList.isNotEmpty
        ? newsList.join('   ✦   ')
        : 'أهلاً بكم في سوق سوريا الشامل 2028 - بوابتكم للتجارة الحرة والآمنة';
    ...
    return Directionality(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: const Color(0xFFD4AF37).withOpacity(0.3), width: 0.8),
        ),
        child: Row(
          children: [
            // 1. شارة عاجل النحيفة والبارزة
            Container(
            ...
            // 2. شريط الأخبار المتحرك ذاتياً بنعومة بدون توقف
            Expanded(
              child: Listener(
            ...
            // 3. كبسولتا أسعار الدولار والذهب المدمجتان على اليسار
            InkWell(
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // ================= قسم روابط التواصل الاجتماعي وفيديو المعاينة =================
                if ((_currentAd.videoUrl != null &&
                        _currentAd.videoUrl!.trim().isNotEmpty) ||
                    ...
                    (_currentAd.tiktokUrl != null &&
                        _currentAd.tiktokUrl!.trim().isNotEmpty) ||
                    (_currentAd.youtubeUrl != null &&
                        _currentAd.youtubeUrl!.trim().isNotEmpty)) ...[
                  Row(
                    children: [
                      Icon(Icons.share,
                          size: 16, color: _manager.secondaryColor),
                      const SizedBox(width: 6),
                      const Text(
                        'روابط التواصل الاجتماعي وفيديو المعاينة:',
                        style: TextStyle(
                    ...
                    children: [
                      ...
                      if (_currentAd.facebookUrl != null &&
                        ...
                        ActionChip(
                          ...
                          side: const BorderSide(
                              color: Color(0xFF1877F2), width: 0.8),
                          onPressed: () =>
                      ...
                      if (_currentAd.instagramUrl != null &&
                        ...
                        ActionChip(
                          ...
                          side: const BorderSide(
                              color: Color(0xFFE1306C), width: 0.8),
                          onPressed: () =>
                      ...
                      if (_currentAd.youtubeUrl != null &&
                        ...
                        ActionChip(
                          ...
                          side: const BorderSide(
                              color: Color(0xFFFF0000), width: 0.8),
                          onPressed: () =>
                              _openSocialLink(_currentAd.youtubeUrl),
                        ),
                      // تليجرام
                        ...
                        ActionChip(
                          ...
                          label: const Text('تليجرام',
                              style: TextStyle(
                                  fontSize: 11, fontWeight: FontWeight.bold)),
                          backgroundColor:
                              const Color(0xFF229ED9).withOpacity(0.12),
                          side: const BorderSide(
                              color: Color(0xFF229ED9), width: 0.8),
                          onPressed: () =>
                      ...
                      if (_currentAd.videoUrl != null &&
                        ...
                        ActionChip(
                          avatar: const Icon(Icons.videocam,
                              color: Color(0xFFD4AF37), size: 18),
                          label: const Text('فيديو المعاينة',
                              style: TextStyle(
                                  fontSize: 11, fontWeight: FontWeight.bold)),
                          backgroundColor:
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      // زر فتح حساب المعلن والانتقال لملفه الشخصي الكامل
                      InkWell(
                        borderRadius: BorderRadius.circular(10),
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (ctx) => UserProfileScreen(
                                targetUserId: _currentAd.userId.isNotEmpty &&
                                        _currentAd.userId != 'guest'
                                    ? _currentAd.userId
                                    : null,
                                targetUserName:
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              const SizedBox(width: 6),
                              const Text(
                                'عرض الملف الشخصي للمعلن وتقييماته 👤',
                                style: TextStyle(
                                  color: Color(0xFFD4AF37),
                                  fontSize: 12.5,
                                  fontWeight: FontWeight.bold,
...
// متصلة لحظياً ومباشرة مع جدول الباقات المركزي في غرفة العمليات
// ==============================================================================
...
class _SubscriptionPlansScreenState extends State<SubscriptionPlansScreen> {
  final AppStateManager _manager = AppStateManager();
  Map<String, dynamic>? _selectedPlanMap;
  ...
  Future<void> _submitUpgrade() async {
    ...
    if (_selectedPlanMap == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('يرجى اختيار باقة للاشتراك بها')),
      );
      return;
    }
    ...
    final durationDays = isYearly ? 365 : 30;
    final durationLabel =
        isYearly ? 'اشتراك سنوي (12 شهر) 🌟' : 'اشتراك شهري (30 يوم) 📅';

    final double basePrice = ((_selectedPlanMap!['price_usdt'] ??
            _selectedPlanMap!['price_usd'] ??
            15) as num)
        .toDouble();
    final finalPrice = isYearly ? (basePrice * 10) : basePrice;
    final planId = _selectedPlanMap!['id']?.toString() ?? 'plan_custom';
    final planName = _selectedPlanMap!['name']?.toString() ?? 'باقة VIP';

    // 1. رفع صورة الإيصال لمستودع التخزين السحابي
    String? receiptUrl;
    try {
      receiptUrl = await StorageUploadService.uploadImageBytes(
        bucketName: kStorageBucketFeedbacks,
    ...
    // 2. إرسال الطلب في جدول subscription_requests (ليظهر فوراً في شاشة الإيصالات بالصورة 1)
    try {
      await Supabase.instance.client.from('subscription_requests').insert({
        ...
        'payment_method':
            _selectedGateway == 'SHAM_CASH' ? 'شام كاش 🇸🇾' : 'بينانس USDT 🪙',
      ...
      debugPrint('Error inserting subscription_requests: $e');
    }

    // 3. إرسال الطلب أيضاً إلى جدول payment_audits لغرفة العمليات المركزية
    ...
    if (mounted) {
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(
            'تم استلام بياناتك وصورة إشعار الدفع بنجاح ($durationLabel).\nسيتم مراجعة الإيصال وتفعيل باقتك مع العداد التنازلي التلقائي فوراً!',
            style: const TextStyle(
  ...
  Widget build(BuildContext context) {
    final isYearly = _selectedDurationType == 'yearly';
    final double basePrice = _selectedPlanMap != null
        ? ((_selectedPlanMap!['price_usdt'] ??
                _selectedPlanMap!['price_usd'] ??
                0) as num)
            .toDouble()
        : 0.0;
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: ChoiceChip(
                    ...
                    labelStyle: TextStyle(
                        color: _selectedDurationType == 'monthly'
                            ? Colors.black
                            : Colors.white),
                    onSelected: (v) =>
                ...
                Expanded(
                  child: ChoiceChip(
                    ...
                    backgroundColor: const Color(0xFF1E293B),
                    labelStyle: TextStyle(
                        color: _selectedDurationType == 'yearly'
                            ? Colors.black
                            : Colors.white),
                    onSelected: (v) =>
            ...
            const SizedBox(height: 16),
            const Text('2. اختر الباقة المناسبة لك (تحديث حي ومباشر):',
                style: TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 14,
                    color: Colors.white)),
            ...
            // 🌟 استدعاء كل الباقات الحية من السيرفر مباشرة وتطابقها التام مع غرفة العمليات
            StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                // اختيار أول باقة افتراضياً إذا لم يتم الاختيار
                if (_selectedPlanMap == null) {
                  final paid = plans.firstWhere(
                    (p) =>
                        ((p['price_usdt'] ?? p['price_usd'] ?? 0) as num) > 0,
                    orElse: () => plans.first,
                  );
                  _selectedPlanMap = paid;
                }

                return Column(
                  children: plans.map((plan) {
                    final isSelected = _selectedPlanMap?['id'] == plan['id'];
                    final colorHex =
                        plan['badge_color'] as String? ?? '#D4AF37';
                    ...
                    final iconKey = plan['icon_key'] ??
                        plan['icon_name'] ??
                        'workspace_premium';
                    final iconData = _getIconFromKey(iconKey);

                    final double planBaseUsd =
                        ((plan['price_usdt'] ?? plan['price_usd'] ?? 0) as num)
                            .toDouble();
                    final itemPriceUsd =
                        isYearly ? (planBaseUsd * 10) : planBaseUsd;
                    final itemPriceSyp =
                        (itemPriceUsd * _manager.exchangeRateUsdToSyp).toInt();

                    final featuresList =
                        (plan['features'] as List?)?.cast<dynamic>() ?? [];
                    final maxAds = plan['max_ads'] ?? 50;
                    final maxImages = plan['max_images_per_ad'] ?? 10;
                    ...
                    return Card(
                      ...
                      child: InkWell(
                        onTap: () => setState(() => _selectedPlanMap = plan),
                        borderRadius: BorderRadius.circular(14),
                        child: Padding(
                          padding: const EdgeInsets.all(12),
                          child: Column(
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          'سقف الإعلانات: $maxAds إعلان • $maxImages صور لكل إعلان',
                                          style: const TextStyle(
            ...
            Container(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('المجموع الإجمالي: \$$finalPriceUsd USD',
                      style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          color: Color(0xFF22C55E),
                          fontSize: 13.5)),
              ...
              children: [
                Expanded(
                  child: ChoiceChip(
                    ...
                    backgroundColor: const Color(0xFF1E293B),
                    labelStyle: TextStyle(
                        color: _selectedGateway == 'SHAM_CASH'
                            ? Colors.black
                            : Colors.white),
                    onSelected: (v) =>
                ...
                Expanded(
                  child: ChoiceChip(
                    ...
                    backgroundColor: const Color(0xFF1E293B),
                    labelStyle: TextStyle(
                        color: _selectedGateway == 'BINANCE_USDT'
                            ? Colors.black
                            : Colors.white),
                    onSelected: (v) =>
                ...
                child: _receiptImageBytes != null
                    ...
                    : const Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 6),
                          Text('اضغط لاختيار صورة إيصال التحويل من المعرض',
                              style: TextStyle(
                                  fontWeight: FontWeight.bold,
                                  fontSize: 13,
                                  color: Colors.white)),
...
// 🌟 القائمة الجانبية السيادية المتقدمة (الثلاث شخطات)
      drawer: Drawer(
        ...
        child: Directionality(
          ...
          child: Column(
            children: [
              // رأس القائمة الفخم مع الشعار والعلم
              Container(
                width: double.infinity,
                padding: const EdgeInsets.only(
                    top: 48, bottom: 20, right: 16, left: 16),
                ...
                child: Column(
                  ...
                  children: [
                    const Row(
                      children: [
                        SyrianIndependenceFlag(width: 28, height: 18),
                        SizedBox(width: 10),
                        Text(
                    ...
                    const SizedBox(height: 8),
                    Text(
                      'المنصة الوطنية الكبرى للبيع والشراء والتجارة الموثوقة 🇸🇾',
                      style: TextStyle(
                        color: Colors.white70,
                        fontSize: 11,
                      ),
...
// قائمة العناصر الفخمة
              Expanded(
                child: ListView(
                  padding:
                      const EdgeInsets.symmetric(vertical: 8, horizontal: 8),
                  children: [
                    // 1. باقات الاشتراك VIP (تفتح صفحة الباقات والترقية الحقيقية فوراً)
                    ListTile(
                      shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(10)),
                      leading: const Icon(Icons.workspace_premium_rounded,
                          color: Color(0xFFD4AF37), size: 24),
                      title: const Text('باقات الترقية والاشتراك VIP 👑',
                          style: TextStyle(
                              color: Colors.white,
                              fontSize: 13.5,
                              fontWeight: FontWeight.bold)),
                      subtitle: const Text(
                          'تمييز الإعلانات، تثبيت وحجز البانوراما',
                          style:
                    ...
                    // 2. دليل المكاتب العقارية المعتمدة
                    ListTile(
                      shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(10)),
                      ...
                      title: const Text('دليل المكاتب العقارية المعتمدة 🏢',
                          style: TextStyle(
                              ...
                              fontWeight: FontWeight.bold)),
                      subtitle: const Text(
                          'مكاتب مرخصة وموثوقة لبيع وشراء العقارات',
                          style:
                              TextStyle(color: Colors.white60, fontSize: 10.5)),
                      trailing: const Icon(Icons.arrow_forward_ios_rounded,
                    ...
                    // 3. دليل معارض السيارات المعتمدة
                    ListTile(
                      shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(10)),
                      leading: const Icon(Icons.directions_car_filled_rounded,
                          color: Color(0xFFF59E0B), size: 24),
                      title: const Text('دليل معارض السيارات المعتمدة 🚗',
                          style: TextStyle(
                              color: Colors.white,
                              fontSize: 13.5,
                              fontWeight: FontWeight.bold)),
                      subtitle: const Text(
                          'أفضل صالات وتجار السيارات المعتمدين',
                          style:
                              TextStyle(color: Colors.white60, fontSize: 10.5)),
                      trailing: const Icon(Icons.arrow_forward_ios_rounded,
                          color: Colors.white38, size: 14),
                    ...
                    // 4. تواصل مع الإدارة وصوتك مسموع
                    ListTile(
                      shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(10)),
                      leading: const Icon(Icons.headset_mic_rounded,
                          color: Color(0xFFD4AF37), size: 24),
                      title: const Text('تواصل مع إدارة السوق وصوتك مسموع 💬',
                          style: TextStyle(
                              color: Colors.white,
                              fontSize: 13,
                              fontWeight: FontWeight.w600)),
                      subtitle: const Text('اقتراحاتك وملاحظاتك تهمنا مباشرة',
                          style:
                              TextStyle(color: Colors.white60, fontSize: 10.5)),
                      trailing: const Icon(Icons.arrow_forward_ios_rounded,
                          color: Colors.white38, size: 14),
                    ...
                    // 5. غرفة العمليات والإشراف المركزي (مقفل حصراً بإيميلي عبده عواد وسامر عواد)
                    Builder(
                      builder: (context) {
                        final currentEmail =
                            (_manager.currentUserEmail).trim().toLowerCase();
                        final isOwnerAdmin =
                            currentEmail == 'aoaadabdo@gmail.com' ||
                                currentEmail == 'sameraoaad@gmail.com';
                        ...
                        return Column(
                          ...
                          children: [
                            ...
                            ListTile(
                              ...
                              title: const Text(
                                  'طلبات توثيق المكاتب والاشتراكات 💎',
                                  style: TextStyle(
                                      color: Colors.white,
                                      fontSize: 12.5,
                                      fontWeight: FontWeight.bold)),
                              subtitle: const Text(
                                  'تدقيق إيصالات شام كاش و بايننس وتفعيل الغرف',
                                  style: TextStyle(
                                      color: Colors.white60, fontSize: 10)),
                              trailing: const Icon(
                                  Icons.arrow_forward_ios_rounded,
                              ...
                              onTap: () {
                                ...
                                Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                      builder: (ctx) =>
                                          const AgencyRequestsAdminScreen()),
                                );
                              },
                            ),
                          ],
      ...
      appBar: AppBar(
        ...
        titleSpacing: 0,
        leading: Builder(
          builder: (ctx) => _currentNavIndex != 0
              ? IconButton(
        ...
        actions: [
          // ⚙️ ترس الإعدادات الشاملة وحسابي والمظهر
          IconButton(
                        ...
                        children: [
                          ...
                          Card(
                            ...
                            child: ListTile(
                              ...
                              subtitle: const Text(
                                'تعديل البيانات، رقم الهاتف، إعلاناتي النشطة',
                                style: TextStyle(
                          ...
                          ListTile(
                            ...
                            title: const Text('الوضع الليلي / النهاري',
                                style: TextStyle(
                                    ...
                                    fontWeight: FontWeight.w600,
                                    color: Colors.white)),
                            subtitle: const Text('تبديل مظهر الإضاءة والألوان',
                                style: TextStyle(
                            ...
                            onTap: () {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('تم تبديل الوضع بنجاح ✨'),
                          ...
                          const SizedBox(height: 8),
                          // 3. تواصل مع الإدارة وصوتك مسموع
                          ListTile(
                            shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(12)),
                            tileColor: const Color(0xFF1E293B),
                            ...
                            title: const Text('تواصل مع الإدارة وصوتك مسموع 💬',
                                style: TextStyle(
                                    fontSize: 13,
                                    fontWeight: FontWeight.w600,
                                    color: Colors.white)),
                            subtitle: const Text('اقتراحاتك وملاحظاتك تهمنا',
                                style: TextStyle(
                                    fontSize: 11, color: Colors.white60)),
                            trailing: const Icon(
                                Icons.arrow_forward_ios_rounded,
                          ...
                          // 4. غرفة العمليات والإشراف المركزي (محصورة فقط وفقط بالحسابين المعتمدين)
                          Builder(
                            builder: (context) {
                              final currentUser =
                              ...
                              final bool isSuperAdmin =
                                  authorizedAdmins.contains(currentEmail);

                              // إذا لم يكن أحد هذين الحسابين، تختفي تماماً ولا تظهر لأحد!
                              ...
                              return Column(
                                children: [
                                  ...
                                  ListTile(
                                    ...
                                    tileColor: const Color(0xFFD4AF37)
                                        .withOpacity(0.15),
                                    leading: const Icon(
                                        Icons.admin_panel_settings_rounded,
                                        color: Color(0xFFD4AF37)),
                                    title: const Text(
                                        ...
                                        style: TextStyle(
                                            ...
                                            color: Color(0xFFD4AF37))),
                                    subtitle: const Text(
                                        'صلاحيات الإدارة والتحكم بالسوق',
                                        style: TextStyle(
                                            fontSize: 11,
                                            color: Colors.white70)),
                                    trailing: const Icon(
              ...
              showModalBottomSheet(
                ...
                builder: (ctx) => Padding(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'ابحث في إعلانات السوق...',
      ...
      body: SafeArea(
        child: IndexedStack(
          ...
          children: [
            _buildHomeFeedTab(), // 0: الرئيسية
            _buildDepartmentsTab(), // 1: الأقسام والفروع
            _buildFavoritesTab(), // 2: المفضلة
            _buildCompaniesDirectoryTab(), // 3: دليل الشركات والمعارض المعتمدة 🏢
          ],
        ),
      ),
      ...
      bottomNavigationBar: BottomAppBar(
        ...
        child: Row(
          children: [
            ...
            // مساحة مخصصة للزر الأوسط لعدم حجب أيقونة الإضافة
            const SizedBox(width: 48),

            // 3. زر المفضلة (Index 2)
            Expanded(
              child: InkWell(
...
// ==============================================================================
  // ⚡ الشريط الموحد فائق الرشاقة: يدمج الأخبار وأسعار الصرف والذهب في سطر واحد (30px فقط)
  // ==============================================================================
  Widget _buildUnifiedUltraCompactTopBar() {
    final newsList = _manager.newsTicker;
    final newsText = newsList.isNotEmpty
        ? newsList.join('   ✦   ')
        : 'أهلاً بكم في سوق سوريا الشامل 2028';
    final usd = _manager.exchangeRateUsdToSyp.toInt();
    final gold = _manager.goldPrice21kSyp.toInt();
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            // 2. شريط الأخبار المتحرك في المساحة المتبقية
            Expanded(
              child: Listener(
            ...
            // 3. كبسولتا أسعار الصرف والذهب المدمجتان على اليسار
            InkWell(
  ...
  // ===========================================================================
  // ويدجت شاشة الأقسام الكاملة مع الشريط الموحد الجديد
  // ===========================================================================
  Widget _buildDepartmentsTab() {
    return Column(
      children: [
        _buildUnifiedUltraCompactTopBar(),
          ...
          action: SnackBarAction(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (ctx) => SupabaseAuthScreen(
                    onAuthSuccess: () {
                      if (mounted) setState(() {});
                    },
                  ),
  ...
  Future<void> _pickAndUploadBannerDirectly() async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ElevatedButton(
            ...
            child: const Text('القسم الأيمن / العلوي 🔲 (Slot 1)',
                style: TextStyle(fontWeight: FontWeight.bold)),
          ),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
            ...
            child: const Text('القسم الأيسر / السفلي 🔲 (Slot 2)',
                style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    ...
    try {
      ...
      // استخدام مستودع البانوراما الرسمي الموحد
      ...
      if (uploadedUrls.isNotEmpty) {
        final bannerId = 'bn_${DateTime.now().millisecondsSinceEpoch}';
        final newBanner = BannerItem(
          id: bannerId,
          imageUrls: uploadedUrls,
          title: selectedSlot == 1
              ? 'إعلان مميز (القسم الأول) ✨'
              : 'عرض خاص (القسم الثاني) 🚀',
          subtitle: 'سوق سوريا الشامل',
          description: 'تم النشر عبر لوحة الإشراف المركزي',
        ...
        try {
          ...
          debugPrint('✅ تم حفظ البانوراما بالسيرفر بنجاح!');
        } catch (serverErr) {
          debugPrint('❌ تنبيه سيرفر البانوراما: $serverErr');
        }
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                  '✅ تم رفع ونشر البانوراما للقسم رقم ($selectedSlot) بنجاح بالسيرفر!'),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  '⚠️ تعذر رفع الصور إلى المستودع السحابي، تأكد من الاتصال.'),
              backgroundColor: Colors.orange,
            ),
          );
        }
  ...
  Widget _buildHomeFeedTab() {
    ...
    final bottomBanners = _manager.banners
        .where((b) => b.slot == 2 && b.isActive && !b.isExpired)
        .toList();
    final bool showTop = _manager.isBannerSlot1Visible && topBanners.isNotEmpty;
    final bool showBottom =
        _manager.isBannerSlot2Visible && bottomBanners.isNotEmpty;

    return Directionality(
      textDirection: TextDirection.rtl,
      child: Column(
        children: [
          // 1. شريط أسعار الصرف النظيف
          ...
          // 3. 🌟 البانوراما العلوية (Slot 1) - ثابتة دائماً في الأعلى لا تختفي
          if (showTop)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              child: CinemaUltraLuxuryPanoramaWidget(
                key: const ValueKey('panorama_slot_1_fixed'),
                slot: 1,
                banners: topBanners,
                intervalSeconds: _manager.bannerDefaultIntervalSeconds,
              ),
            ),

          // 4. 📜 منطقة المنشورات الحرة القابلة للتمرير (تتحرك معها البانوراما السفلية Slot 2)
          Expanded(
            child: RefreshIndicator(
              ...
              child: CustomScrollView(
                ...
                slivers: [
                  // 💎 البانوراما السفلية (Slot 2) - تتدفق وتتمرر بانسيابية مع المنشورات
                  if (showBottom)
                    SliverToBoxAdapter(
                      child: Padding(
                        padding: const EdgeInsets.fromLTRB(8, 4, 8, 6),
                        child: CinemaUltraLuxuryPanoramaWidget(
                          key: const ValueKey('panorama_slot_2_scrollable'),
                          slot: 2,
                          ...
                          Row(
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color:
                                      _manager.secondaryColor.withOpacity(0.18),
                                  borderRadius: BorderRadius.circular(8),
                                ),
                                child: Text(
                  ...
                  // شبكة المنشورات المتدفقة بكامل حرية الشاشة
                  if (_isLoadingAds)
                    ...
                    SliverPadding(
                      ...
                      sliver: SliverList(
                        delegate: SliverChildBuilderDelegate(
                          ...
                          childCount:
                              filteredAds.length + (_isLoadingMore ? 1 : 0),
                        ),
                      ),
                    ),
                ],
  ...
  Widget _buildRoyalBannersSection() {
    ...
    final topBanners = _manager.banners.where((b) {
      ...
      return notExpired && geoMatch && (b.slot == 1 || b.slot == 0);
    }).toList();

    final bottomBanners = _manager.banners.where((b) {
      final notExpired = !b.isExpired && b.isActive;
      final geoMatch = b.location == 'كل المحافظات' ||
          targetGovernorate == 'كل المحافظات' ||
          b.location == targetGovernorate;
    ...
    final showTop = _manager.isBannerSlot1Visible && topBanners.isNotEmpty;
    final showBottom =
        _manager.isBannerSlot2Visible && bottomBanners.isNotEmpty;
  ...
  Widget _buildAnimatedPanoramaCard(BannerItem banner,
      {required String badgeTitle}) {
    final displayImg =
        banner.imageUrls.isNotEmpty ? banner.imageUrls.first : banner.imageUrl;
          ...
          children: [
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Text(
                  banner.badgeText.isNotEmpty ? banner.badgeText : badgeTitle,
                  style: const TextStyle(
                    fontSize: 9.5,
                    fontWeight: FontWeight.bold,
                    color: Color(0xFF0F172A),
  ...
  // 🌟 بطاقة المنشورات الأفقية الملكية فائقة التوفير للمساحة (بنفس دوال AdItem وSupabase الأصلية)
        ...
        child: InkWell(
          ...
          onTap: () {
            ...
            try {
              Supabase.instance.client
                  .from('ads')
                  .update({'views_count': ad.viewsCount + 1}).eq('id', ad.id);
            } catch (_) {}
                ...
                builder: (ctx) => FullAdDetailsScreen(
                  ...
                  onAdDeleted: (deletedId) async {
                    ...
                    try {
                      ...
                      debugPrint('Error deleting ad from Supabase: $e');
                    }
                  },
                ),
              ),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                // 1. صورة الإعلان المربعة المدمجة مع كافة الشارات
                ClipRRect(
                  borderRadius: BorderRadius.circular(10),
                  child: SizedBox(
                    width: 82,
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        // تظليل ناعم لحماية النصوص والمؤشرات
                        Positioned.fill(
                          child: DecoratedBox(
                            decoration: BoxDecoration(
                              gradient: LinearGradient(
                ...
                // 2. تفاصيل المنشور بوضوح فائق ومريح للعين
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          if (ad.priceUsd != null && ad.priceUsd! > 0)
                            Text(
                              '\$${ad.priceUsd!.toStringAsFixed(0)} USD',
                              style: const TextStyle(
                                color: Color(0xFF22C55E),
                                fontWeight: FontWeight.bold,
                                fontSize: 12,
                          ...
                          if (ad.priceSyp != null && ad.priceSyp! > 0) ...[
                            const SizedBox(width: 6),
                            Text(
                              '(${ad.priceSyp!.toStringAsFixed(0)} ل.س)',
                              style: const TextStyle(
  ...
  Widget _buildChatsAndNegotiationsTab() {
    // التحقق من تسجيل الدخول الحقيقي عبر Supabase أو المدير
    final isActuallyLoggedIn = _manager.isLoggedIn ||
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              const Text(
                'يرجى تسجيل الدخول للوصول إلى رسائلك وعروض التفاوض الفورية.',
                textAlign: TextAlign.center,
...
// ==============================================================================
  // 🏢 شاشة دليل الشركات المعتمدة الحقيقية (المكاتب العقارية + معارض السيارات)
  // ==============================================================================
  Widget _buildCompaniesDirectoryTab() {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          toolbarHeight: 52,
          // شريط أصحاب المكاتب والتوثيق الممول في التولبار
          titleSpacing: 0,
          title: Container(
            ...
            child: Row(
              children: [
                ...
                const Expanded(
                  child: Text(
                    'أصحاب المكاتب والمعارض: وثق شركتك رسمياً',
          ...
          // التاب بار موضوع رسمياً في مكانه الطبيعي (bottom) بارتفاع حر لا يمكن أن يفيض أبداً
          bottom: const TabBar(
            ...
            tabs: [
              ...
              Tab(
                icon: Icon(Icons.directions_car_filled_rounded, size: 18),
                text: 'معارض السيارات 🚗',
              ),
            ],
          ),
        ...
        body: TabBarView(
          children: [
            // 1. مكاتب العقارات المعتمدة الحقيقية من السيرفر
            _buildRealCompaniesList(category: 'مكتب عقاري'),

            // 2. معارض السيارات المعتمدة الحقيقية من السيرفر
            _buildRealCompaniesList(category: 'معرض سيارات'),
          ],
        ),
      ),
    );
  ...
  // جلب الشركات الحقيقية المعتمدة فقط من السيرفر
  Widget _buildRealCompaniesList({required String category}) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        // في حال عدم وجود شركات معتمدة بعد في السيرفر
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  Text(
                    'لا توجد $category معتمدة حالياً',
                    style: const TextStyle(
                        color: Colors.white70,
                        fontSize: 13,
                        fontWeight: FontWeight.bold),
                  ...
                  const SizedBox(height: 6),
                  const Text(
                    'هل تملك مكتباً أو صالة عرض؟ اضغط على زر "طلب توثيق ممول" بالأعلى لتوثيق شركتك واعتمادها رسمياً في المنصة.',
                    textAlign: TextAlign.center,
                    style: TextStyle(color: Colors.white38, fontSize: 11),
                  ),
                ],
        ...
        // عرض الشركات الحقيقية فقط وفتح غرفتها الخاصة عند النقر
        return ListView.builder(
          padding: const EdgeInsets.all(12),
          ...
          itemBuilder: (context, index) {
            ...
            return Card(
              ...
              child: InkWell(
                borderRadius: BorderRadius.circular(12),
                onTap: () {
                  // فتح الغرفة الخاصة المستقلة للمكتب أو المعرض
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (ctx) => AgencyRoomDetailScreen(agencyData: c),
                    ),
                  );
                },
                child: ListTile(
  ...
  Widget _buildProfileTab() {
    ...
    // 🌟 حساب مجموع إجمالي الإعجابات والتقييمات التراكمية لحساب المستخدم عبر كافة إعلاناته
    final int totalAccountLikes = userAds.fold<int>(
      _manager.currentUserPositiveLikes,
      (sum, item) => sum + item.sellerPositiveLikes,
    );

    // حساب معدل النجوم التراكمي (افتراضي ممتاز 4.9 أو بناءً على التقييمات)
    final double cumulativeStars = totalAccountLikes > 0
        ? (4.5 + (totalAccountLikes % 5) * 0.1).clamp(1.0, 5.0)
        : 5.0;

    // تمييز الحساب العادي عن الحساب الموثق / المدفوع
    final bool isPaidOrVerified = _manager.isCurrentUserVerified ||
        currentPlan.name.contains('مدفوع') ||
        currentPlan.name.contains('VIP') ||
        currentPlan.name.contains('بلاتيني') ||
        currentPlan.name.contains('ذهبي');
      ...
      children: [
            ...
            children: [
              Row(
                children: [
                  CircleAvatar(
                    radius: 32,
                    backgroundColor: isPaidOrVerified
                        ? const Color(0xFFD4AF37)
                        : _manager.secondaryColor,
                    child: Text(
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            const SizedBox(width: 6),
                            // 🌟 شارات الحسابات: تمييز الحساب الموثق/المدفوع عن الحساب العادي
                            if (isPaidOrVerified)
                              Container(
                                padding: const EdgeInsets.symmetric(
                                    horizontal: 6, vertical: 2),
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  ...
                                  const SizedBox(width: 4),
                                  Text('$totalAccountLikes إعجاب تراكمي',
                                      style: const TextStyle(
                                          color: Colors.white,
                                          fontSize: 11,
                                          fontWeight: FontWeight.bold)),
              ...
              const SizedBox(height: 14),
              // 🌟 بطاقة التقييم التراكمي الذهبي الشامل المجمع من كافة المنشورات
              Container(
                padding:
        ...
        if (!_manager.isLoggedIn)
          ListTile(
            shape:
                RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
            tileColor: const Color(0xFFD4AF37).withOpacity(0.1),
            leading: const Icon(Icons.login_rounded, color: Color(0xFFD4AF37)),
            title: const Text('تسجيل الدخول / إنشاء حساب جديد',
                style: TextStyle(
                    fontWeight: FontWeight.bold, color: Colors.white)),
            subtitle: const Text(
                'تسجيل سريع ومحمي عبر Supabase مع استرجاع كلمة المرور',
                style: TextStyle(color: Colors.white60, fontSize: 11)),
            trailing: const Icon(Icons.arrow_forward_ios_rounded,
                size: 14, color: Color(0xFFD4AF37)),
            onTap: () {
        ...
        ListTile(
          ...
          subtitle: const Text('ميزات حصرية ونشر غير محدود للمكاتب والشركات',
              style: TextStyle(color: Colors.white60, fontSize: 11)),
          trailing: const Icon(Icons.arrow_forward_ios_rounded,
              size: 14, color: Color(0xFFD4AF37)),
          onTap: () {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(builder: (ctx) => const VipPackagesScreen()),
            );
          },
        ),
        // حماية غرفة العمليات والإشراف المركزي (مستحيل تظهر إلا لإيميلي عبده عواد وسامر عواد حصراً)
        Builder(
          builder: (context) {
            final currentEmail =
                (Supabase.instance.client.auth.currentUser?.email ??
                    ...
                    .trim()
                    .toLowerCase();
            final isAuthorizedOwner = currentEmail == 'aoaadabdo@gmail.com' ||
                currentEmail == 'sameraoaad@gmail.com';

            if (!isAuthorizedOwner) return const SizedBox.shrink();
            ...
            return Column(
              ...
              children: [
                ...
                ListTile(
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(10)),
                  tileColor: const Color(0xFFD4AF37).withOpacity(0.12),
                  ...
                  title: const Text('غرفة العمليات ولوحة تحكم المشرفين 🛡️',
                      style: TextStyle(
                          ...
                          color: Color(0xFFD4AF37))),
                  subtitle: const Text(
                      'موافقة الإعلانات، تدقيق إيصالات شام كاش وبينانس، وإدارة الأسعار',
                ...
                ListTile(
                  ...
                  leading: const Icon(Icons.receipt_long_rounded,
                      color: Color(0xFFD4AF37), size: 22),
                  title: const Text(
                      'إدارة الباقات والاشتراكات وتدقيق الإيصالات 💎',
                      style: TextStyle(
                          color: Colors.white,
                          fontSize: 12.5,
                          fontWeight: FontWeight.bold)),
                  subtitle: const Text(
                      'تعديل أسعار الباقات الأربعة وقبول إيصالات شام كاش وبايننس',
              ...
              SnackBar(
                content: Text(
                  _manager.isSuperAdmin
                      ...
                      : '⏳ تم إرسال إعلانك بنجاح إلى غرفة العمليات وسيعرض للجميع فور موافقة الإدارة عليه.',
                ),
                backgroundColor: _manager.primaryColor,
              ),
            );
...
class _FullAddAdScreenState extends State<FullAddAdScreen> {
  ...
  Future<void> _pickImages() async {
    ...
    try {
      // 🌟 جودة معتدلة ومثالية (85% وأبعاد 1280px) تمنع التغبيش وتضمن نقاء التفاصيل تماماً
      final pickedList = await _picker.pickMultiImage(
        imageQuality: 85,
        maxWidth: 1280,
      );
  ...
  Future<void> _submitAd() async {
    ...
    // فحص المعرّف الحقيقي للمستخدم من Supabase Auth لتفادي خطأ الـ UUID
    final authUser = Supabase.instance.client.auth.currentUser;
    final String validUserId = authUser != null
        ? authUser.id
    ...
    try {
      final payload = adItem.toMap();
      // إزالة معرّف المستخدم إذا لم يكن UUID صحيحاً حتى لا يرفضه السيرفر
      if (validUserId.isEmpty) {
        payload.remove('user_id');
      }

      if (widget.initialAd != null && widget.initialAd!.id.isNotEmpty) {
        await Supabase.instance.client
            .from('ads')
            .update(payload)
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('⚠️ تعذر إتمام النشر: $e'),
            backgroundColor: const Color(0xFFDC2626),
          ),
        );
      }
...
class _FullAdminPanelScreenState extends State<FullAdminPanelScreen>
      ...
      children: [
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Row(
                  children: [
                    Icon(Icons.layers, color: Color(0xFFD4AF37), size: 22),
                    SizedBox(width: 8),
                    Text(
                      'غرفة العمليات • التحكم ببانوراما القسمين (فوق بعضهما)',
                      style: TextStyle(
                        color: Colors.white,
                        fontWeight: FontWeight.bold,
                        fontSize: 13.5,
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          Row(
                            children: [
                              Text(_manager.isBannerSlot1Visible ? '🟢' : '🔴',
                          ...
                          ElevatedButton(
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor: _manager.isBannerSlot1Visible
                      ...
                      Row(
                        ...
                        children: [
                          Row(
                            children: [
                              Text(_manager.isBannerSlot2Visible ? '🟢' : '🔴',
                                  style: const TextStyle(fontSize: 14)),
                              const SizedBox(width: 8),
                              const Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                          ...
                          ElevatedButton(
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor: _manager.isBannerSlot2Visible
                                  ? const Color(0xFF0284C7)
                                  : const Color(0xFFE11D48),
                              padding: const EdgeInsets.symmetric(
                                  horizontal: 10, vertical: 5),
                ...
                // 1️⃣ سلايدر سرعة البانوراما العلوية (Slot 1) - ذهبي ومستقل
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Row(
                      children: [
                        Icon(Icons.speed, size: 16, color: Color(0xFFD4AF37)),
                        SizedBox(width: 6),
                        Text('سرعة البانوراما العلوية (Slot 1):',
                            style: TextStyle(
          ...
          children: [
            Expanded(
              child: ElevatedButton.icon(
                ...
                onPressed: () => _showAddCustomBannerDialog(defaultSlot: 1),
              ),
            ),
            const SizedBox(width: 8),
            Expanded(
              child: ElevatedButton.icon(
                ...
                label: const Text('رفع للسفلية (2) 🚀',
                    style:
                        TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                onPressed: () => _showAddCustomBannerDialog(defaultSlot: 2),
              ),
            ),
          ],
        ),
        ...
        Row(
          ...
          children: [
            ...
            Text(
              _manager.isBannerSlot1Visible ? '🟢 معروضة' : '🔴 مخفية',
              style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.bold,
                color:
                    _manager.isBannerSlot1Visible ? Colors.green : Colors.red,
              ),
            ),
          ],
        ),
        ...
        Row(
          ...
          children: [
            ...
            Text(
              _manager.isBannerSlot2Visible ? '🟢 معروضة' : '🔴 مخفية',
              style: TextStyle(
                fontSize: 11,
                fontWeight: FontWeight.bold,
                color:
                    _manager.isBannerSlot2Visible ? Colors.green : Colors.red,
              ),
            ),
          ],
        ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 2),
                  Text(
                    'القسم: ${b.slot == 1 ? "العلوي (1)" : "السفلي (2)"} • الصور: ${b.imageUrls.length}',
            ...
            IconButton(
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  setState(() {
                    final idx =
                        _manager.banners.indexWhere((x) => x.id == b.id);
                    if (idx != -1) {
                      _manager.banners[idx] = BannerItem(
                        id: b.id,
                        ...
                        displayDurationSeconds: b.displayDurationSeconds,
                  ...
                  debugPrint('Error swapping slot: $e');
                }
              },
            ),
            IconButton(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: (selectedImages.isEmpty || isUploading)
                            ...
                            : () async {
                                ...
                                final int targetSlot = defaultSlot;
                                final bannerId =
                                    'bn_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Widget _buildOverviewTab() {
    return ListView(
      ...
      children: [
        ...
        // 👑 جناح إدارة وتخصيص الباقات المركزية وتدقيق الإيصالات
        Container(
          decoration: BoxDecoration(
            gradient: const LinearGradient(
          ...
          child: ListTile(
            ...
            title: const Text(
              'جناح إدارة وتخصيص الباقات والخطط 👑',
              style: TextStyle(
                  color: Colors.white,
                  fontSize: 13.5,
                  fontWeight: FontWeight.bold),
            ...
            subtitle: const Padding(
              ...
              child: Text(
                'إضافة باقة جديدة، تغيير الأسعار، تعديل الميزات والشعارات والأيقونات، وتدقيق الإيصالات فورياً',
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (ctx) => const AdminSubscriptionsManagementScreen(),
                ),
              );
            },
          ),
  ...
  // 🌟 نافذة كتابة سبب الرفض وإرساله لصاحب المنشور
  void _showRejectAdDialog(AdItem ad) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'اكتب سبب الرفض للإعلان (${ad.title})، وسيظهر بوضوح في حساب المعلن ليعرف السبب ويعدله:',
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                hintText:
                    'مثال: السعر غير واقعي، الصور غير واضحة، تفاصيل ناقصة...',
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              final reason = reasonController.text.trim().isNotEmpty
                  ...
                  : 'مخالف لسياسة وشروط النشر المعتمدة';
              Navigator.pop(ctx);
              ...
              await _manager.rejectAd(ad.id, reason);
              if (mounted) {
                setState(() {});
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('❌ تم رفض الإعلان وحفظ سبب الرفض: "$reason"'),
                    backgroundColor: Colors.red.shade800,
                  ),
                );
              }
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        ...
                        icon: const Icon(Icons.cancel_rounded,
                            color: Colors.white, size: 18),
                        label: const Text('رفض مع ذكر السبب ❌',
                            style: TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.bold,
                                fontSize: 12)),
                        onPressed: () => _showRejectAdDialog(ad),
                      ),
                    ),
                  ],
                ),
    ...
    return StatefulBuilder(
      builder: (ctx, setTabState) => ListView(
        ...
        children: [
          ...
          const SizedBox(height: 14),
          const Text('1. سعر صرف الدولار مقابل الليرة السورية (USD/SYP):',
          ...
          const SizedBox(height: 12),
          const Text('2. سعر غرام الذهب عيار 21:',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
          const SizedBox(height: 6),
          TextField(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              final usdRate = double.tryParse(usdBuyCtrl.text.trim()) ?? 15200;
              final goldPrice =
                  double.tryParse(goldSypCtrl.text.trim()) ?? 980000;

              // 🌟 الحفظ السحابي النظيف المحصن ضد أي عمود ناقص في السيرفر
              bool savedSuccessfully = false;
              try {
                savedSuccessfully = await _manager.updateExchangeRatesInCloud(
                  newUsdRate: usdRate,
                  newGoldPrice: goldPrice,
                );
              } catch (e) {
                debugPrint('Rates cloud update error: $e');
              }
              ...
              if (mounted) {
                if (savedSuccessfully) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                          '✅ تم حفظ ونشر أسعار العملات والذهب لجميع الأجهزة بالسيرفر بنجاح ودون أخطاء!'),
                      backgroundColor: Colors.green,
                      duration: Duration(seconds: 3),
                    ),
                  );
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                          '⚠️ تم حفظ الأسعار بالذاكرة المحلية لجميع الأجهزة!'),
                      backgroundColor: Colors.orange,
                    ),
                  );
                }
...
void main() async {
  ...
  // 🛡️ درع سيادي شامل: منع رسم الخطوط الصفراء والسوداء (Overflow Stripes) نهائياً على كل الشاشات
  ErrorWidget.builder = (FlutterErrorDetails details) {
    final bool isOverflow =
        details.exceptionAsString().contains('overflowed by');
    if (isOverflow) {
      // إخفاء الخطوط الصفراء تماماً وعرض مساحة فارغة شفافة بدلاً منها ليبقى شكل التطبيق فخماً
      return const SizedBox.shrink();
    }
    return Container(
      color: const Color(0xFF0F172A),
      alignment: Alignment.center,
      child:
          const Icon(Icons.refresh_rounded, color: Color(0xFFD4AF37), size: 24),
    );
  };
  ...
  FlutterError.onError = (FlutterErrorDetails details) {
    final bool isOverflow =
        details.exceptionAsString().contains('overflowed by');
    if (isOverflow) {
      // كتم طباعة خطأ تجاوز المساحة
      return;
    }
    FlutterError.presentError(details);
  };

  // تهيئة Supabase مباشرة مع حماية Timeout لضمان الربط السحابي الفوري دون تعطيل الإقلاع
  try {
    ...
    debugPrint('⚠️ Supabase Quick Init notice: $e');
  }

  // إقلاع فوري للتطبيق وفتح الشاشة الرئيسية مباشرة
...
// حماية الخطوط من الانفجار عند تغيير إعدادات حجم خط الجوال
...
class _ApplyAgencyScreenState extends State<ApplyAgencyScreen> {
  ...
  String _category = 'مكتب عقاري';
  String _planName = 'باقة VIP الذهبية (15\$ / شهر)';
  String _paymentMethod = 'شام كاش (Sham Cash)';
  ...
  // رفع صورة وصل التحويل إلى سيرفر Supabase Storage
  Future<void> _pickAndUploadReceipt() async {
    ...
    try {
      ...
      final fileName =
          'receipts/${DateTime.now().millisecondsSinceEpoch}_${widget.userId}.$ext';

      await Supabase.instance.client.storage.from('agency_assets').uploadBinary(
            fileName,
            bytes,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ تم رفع صورة وصل التحويل بنجاح!'),
            backgroundColor: Color(0xFF16A34A),
          ),
        );
      }
  ...
  // إرسال الطلب إلى جدول الوكالات بالسيرفر
  Future<void> _submitAgencyApplication() async {
    ...
    if (_receiptImageUrl == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('⚠️ يرجى إرفاق صورة إشعار أو وصل التحويل لإتمام الطلب'),
          backgroundColor: Colors.orange,
        ),
      );
      return;
    ...
    try {
      await Supabase.instance.client.from('agency_rooms').insert({
        ...
        'payment_receipt_url': _receiptImageUrl,
        'is_approved': false, // بانتظار موافقتك من لوحة الإدارة
      });
    ...
    showDialog(
      ...
      builder: (ctx) => Directionality(
        ...
        child: AlertDialog(
          ...
          content: const Text(
            'تم إرسال بيانات وكالتكم وصورة الوصل إلى غرفة الإشراف المركزي.\n\nسيتم التحقق من الوصل واعتماد غرفتكم المستقلة رسمياً خلال وقت وجيز، وستتمكن من ضبط الغلاف والروابط والبدء بنشر إعلاناتك.',
          ...
          actions: [
            ElevatedButton(
              ...
              child: const Text('موافق، تم الاستيعاب',
                  style: TextStyle(
          ...
          child: ListView(
            ...
            children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('مزايا الغرفة المستقلة المعتمدة 🌟',
                              style: TextStyle(
                                  color: Colors.white,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 13.5)),
                          SizedBox(height: 4),
                          Text(
                              'غلاف خاص، شعار رسمي، روابط التواصل (يوتيوب، فيسبوك، تليجرام)، وشارة توثيق سيادية لكل إعلاناتك.',
                              style: TextStyle(
                                  color: Colors.white70,
                                  fontSize: 11,
                                  height: 1.4)),
              ...
              TextFormField(
                ...
                style: const TextStyle(color: Colors.white, fontSize: 13),
                decoration: InputDecoration(
                  labelText: 'اسم المكتب أو المعرض التجاري الرسمي *',
                  labelStyle: const TextStyle(color: Colors.white70),
                  filled: true,
                  fillColor: const Color(0xFF1E293B),
                  border: OutlineInputBorder(
                ...
                children: [
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      style: const TextStyle(color: Colors.white, fontSize: 13),
                      decoration: InputDecoration(
                        labelText: 'المنطقة / الحي (مثال: المزة)',
                        labelStyle: const TextStyle(color: Colors.white70),
                        filled: true,
                        fillColor: const Color(0xFF1E293B),
                        border: OutlineInputBorder(
              ...
              // 5. نبذة عن المكتب وساعات العمل
              TextFormField(
                controller: _bioCtrl,
                maxLines: 2,
                style: const TextStyle(color: Colors.white, fontSize: 13),
                decoration: InputDecoration(
                  labelText: 'نبذة مختصرة عن خدمات المكتب ومواعيد العمل',
                  labelStyle: const TextStyle(color: Colors.white70),
                  filled: true,
                  fillColor: const Color(0xFF1E293B),
                  border: OutlineInputBorder(
              ...
              // ==========================================
              // صندوق معلومات الدفع والاشتراك المالي 💳
              // ==========================================
              Container(
                padding: const EdgeInsets.all(14),
                decoration: BoxDecoration(
                ...
                child: Column(
                  ...
                  children: [
                    const Row(
                      children: [
                        Icon(Icons.payment_rounded,
                            color: Color(0xFFD4AF37), size: 22),
                        SizedBox(width: 8),
                        Text('تفاصيل الاشتراك والدفع المعتمد 🧾',
                            style: TextStyle(
                                color: Color(0xFFD4AF37),
                                fontWeight: FontWeight.bold,
                                fontSize: 14)),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      style: const TextStyle(color: Colors.white, fontSize: 13),
                      decoration: const InputDecoration(
                        labelText: 'طريقة التحويل التي استخدمتها *',
                        labelStyle: TextStyle(color: Colors.white70),
                        border: OutlineInputBorder(),
                      ),
                      items: [
                        'شام كاش (Sham Cash)',
                        'بايننس (Binance USDT TRC20)',
                        'حوالات الهرم أو الفؤاد',
                      ]
                          .map(
                    ...
                    Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          if (_paymentMethod.contains('شام كاش')) ...[
                            const Text('حساب شام كاش المعتمد للإدارة:',
                                style: TextStyle(
                                    color: Colors.white70, fontSize: 11)),
                            const SelectableText(
                                '963944000111 (سوق سوريا الشامل)',
                                style: TextStyle(
                                    color: Color(0xFFD4AF37),
                                    fontWeight: FontWeight.bold,
                                    fontSize: 13)),
                          ] else if (_paymentMethod.contains('بايننس')) ...[
                            const Text('عنوان محفظة USDT (Network: TRC20):',
                                style: TextStyle(
                                    color: Colors.white70, fontSize: 11)),
                            const SelectableText(
                                'TXYZ1234567890MarketSyriaOfficialUSDT',
                                style: TextStyle(
                                    color: Color(0xFF38BDF8),
                                    fontWeight: FontWeight.bold,
                                    fontSize: 12)),
                            ...
                            const Text('حوالة الهرم / الفؤاد باسم:',
                                style: TextStyle(
                                    color: Colors.white70, fontSize: 11)),
                            const Text(
                                'إدارة سوق سوريا 2028 - هاتف: 0944000111',
                                style: TextStyle(
                                    color: Colors.white,
                                    fontWeight: FontWeight.bold,
                                    fontSize: 12.5)),
                    ...
                    // زر رفع صورة إشعار أو وصل التحويل 🧾
                    InkWell(
                        ...
                        child: _isUploadingReceipt
                            ...
                            : Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    _receiptImageUrl != null
                                        ? 'تم إرفاق صورة الوصل بنجاح (انقر للتغيير)'
                                        : 'انقر هنا لإرفاق صورة وصل أو إشعار الدفع 🧾 *',
                                    style: TextStyle(
                                      color: _receiptImageUrl != null
                                          ? const Color(0xFF16A34A)
                                          : Colors.white,
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: _isSubmitting ? null : _submitAgencyApplication,
                  child: _isSubmitting
                      ? const CircularProgressIndicator(color: Colors.black)
                      : const Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            ...
                            SizedBox(width: 8),
                            Text(
                              'إرسال طلب التوثيق والاشتراك للإدارة 🚀',
                              style: TextStyle(
                                  color: Colors.black,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 14),
...
// ==============================================================================
// 🏛️ شاشة الغرفة المستقلة للوكالة / المعرض المعتمد (صفحة الشركة المتكاملة)
// ==============================================================================
class AgencyRoomDetailScreen extends StatelessWidget {
  final Map<String, dynamic> agencyData;

  const AgencyRoomDetailScreen({super.key, required this.agencyData});
  ...
  Widget build(BuildContext context) {
    final String name =
        agencyData['agency_name'] ?? agencyData['name'] ?? 'وكالة معتمدة';
    final String category = agencyData['category'] ?? 'مكتب معتمد';
    final String city = agencyData['city'] ?? '';
    final String district =
        agencyData['district'] ?? agencyData['address'] ?? '';
    final String phone = agencyData['phone'] ?? '';
    final String whatsapp = agencyData['whatsapp'] ?? phone;
    final String bio = agencyData['bio_description'] ??
        'وكالة مرخصة ومعتمدة في سوق سوريا الشامل 2028.';
    final String? coverUrl = agencyData['cover_url'];
    final String? logoUrl = agencyData['logo_url'];

    final String? youtube = agencyData['youtube_url'];
    final String? facebook = agencyData['facebook_url'];
    final String? telegram = agencyData['telegram_url'];
    ...
    return Scaffold(
      ...
      body: Directionality(
        ...
        child: CustomScrollView(
          slivers: [
            // 1. غلاف المكتب العريض والشعار (Cover & Logo)
            SliverAppBar(
              expandedHeight: 200,
              pinned: true,
              backgroundColor: const Color(0xFF0F172A),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // مركز الروابط الاجتماعية (Social Hub)
                    const Text('منصات وروابط المكتب الرسمية:',
                        style: TextStyle(
                            color: Colors.white70,
                            fontSize: 12,
                            fontWeight: FontWeight.bold)),
                      ...
                      children: [
                        if (youtube != null && youtube.isNotEmpty)
                          InkWell(
                            ...
                            child: _buildSocialIcon(Icons.video_library_rounded,
                                const Color(0xFFEF4444), 'يوتيوب'),
                          ),
                        if (facebook != null && facebook.isNotEmpty)
                          InkWell(
                            onTap: () => _launchUrl(facebook),
                            child: _buildSocialIcon(Icons.facebook_rounded,
                                const Color(0xFF3B82F6), 'فيسبوك'),
                          ),
                        if (telegram != null && telegram.isNotEmpty)
                          InkWell(
                            onTap: () => _launchUrl(telegram),
                            child: _buildSocialIcon(Icons.send_rounded,
                                const Color(0xFF0284C7), 'تليجرام'),
                          ),
                      ],
                    ),
                    const Divider(color: Colors.white12, height: 32),

                    // عروض وإعلانات المكتب الحصرية
                    Text(
                      'العروض والإعلانات المنشورة من $name:',
                      style: const TextStyle(
                          color: Color(0xFFD4AF37),
                          fontSize: 13.5,
                          fontWeight: FontWeight.bold),
            ...
            SliverFillRemaining(
              ...
              child: Center(
                child: Padding(
                  padding: const EdgeInsets.all(24.0),
                  child: Text(
                    'لا توجد إعلانات معروضة حالياً لهذا المكتب',
                    style: TextStyle(
...
// ==============================================================================
// 🛡️ شاشة الإشراف المركزي: إدارة المشتركين وتدقيق الإيصالات وتوثيق المكاتب
// ==============================================================================
class AgencyRequestsAdminScreen extends StatefulWidget {
  const AgencyRequestsAdminScreen({super.key});

  @override
  State<AgencyRequestsAdminScreen> createState() =>
      _AgencyRequestsAdminScreenState();
}

class _AgencyRequestsAdminScreenState extends State<AgencyRequestsAdminScreen>
    with SingleTickerProviderStateMixin {
  late TabController _tabController;
  ...
  // جلب طلبات المكاتب العقارية ومعارض السيارات
  Future<void> _fetchPendingAgencies() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _pendingAgencies = List<Map<String, dynamic>>.from(res);
          _isLoadingAgencies = false;
        });
      }
    } catch (e) {
      if (mounted) setState(() => _isLoadingAgencies = false);
      debugPrint('Error fetching agencies: $e');
    }
  }

  // اعتماد غرفة مكتب
  Future<void> _approveAgency(String id, String agencyName) async {
    try {
      await Supabase.instance.client.from('agency_rooms').update({
        'is_approved': true,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ تم اعتماد وتفعيل غرفة ($agencyName) بنجاح!'),
            backgroundColor: const Color(0xFF16A34A),
          ),
        );
        _fetchPendingAgencies();
      ...
      debugPrint('Error approving agency: $e');
    }
  }
  ...
  Future<void> _deleteAgency(String id) async {
    try {
      await Supabase.instance.client.from('agency_rooms').delete().eq('id', id);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('تم حذف طلب الغرفة بنجاح 🗑️'),
              backgroundColor: Colors.red),
        );
        _fetchPendingAgencies();
      }
  ...
  // 🌟 إضافة أيام مكافأة للمشترك وتحديث السيرفر وملف المشترك فوراً (+3 أو +7 أو +30)
  Future<void> _addBonusDays(Map<String, dynamic> audit, int daysToAdd) async {
    final userId = audit['user_id']?.toString() ?? '';
    final txId = audit['id']?.toString() ?? '';
    final currentDurationHours =
        (audit['duration_hours'] as num?)?.toInt() ?? 720;
    final newDurationHours = currentDurationHours + (daysToAdd * 24);

    try {
      // 1. تحديث تاريخ انتهاء الاشتراك في جدول profiles
      if (userId.isNotEmpty && !userId.startsWith('guest')) {
        final profileRes = await Supabase.instance.client
            .from('profiles')
            .select('plan_expires_at, vip_expires_at')
            .eq('id', userId)
            .maybeSingle();

        DateTime currentExpire = DateTime.now();
        if (profileRes != null && profileRes['plan_expires_at'] != null) {
          try {
            final parsed = DateTime.parse(profileRes['plan_expires_at']);
            if (parsed.isAfter(currentExpire)) currentExpire = parsed;
          } catch (_) {}
        }

        final newExpire = currentExpire.add(Duration(days: daysToAdd));

        await Supabase.instance.client.from('profiles').update({
          'plan_expires_at': newExpire.toIso8601String(),
          'vip_expires_at': newExpire.toIso8601String(),
          'is_verified': true,
        }).eq('id', userId);
      }

      // 2. تحديث السجل في payment_audits ليبقى تحت المراقبة
      await Supabase.instance.client.from('payment_audits').update({
        'duration_hours': newDurationHours,
        'duration_label': 'ممدد (+ $daysToAdd أيام مكافأة)',
      }).eq('id', txId);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                '🎁 تم منح المشترك $daysToAdd أيام مكافأة وتحديث ملفه فورياً!'),
            backgroundColor: const Color(0xFF16A34A),
          ),
        );
      }
    } catch (e) {
      debugPrint('Error adding bonus days: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('⚠️ تعذر إضافة الأيام: $e')),
        );
      }
    }
  }

  // 🌟 تجديد الاشتراك لـ 30 يوماً إضافية
  Future<void> _renewSubscription(Map<String, dynamic> audit) async {
    await _addBonusDays(audit, 30);
  }

  // 🌟 حذف اشتراك المشترك نهائياً من السيرفر وبطاقة التحكم
  Future<void> _deleteSubscription(String txId) async {
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'هل أنت متأكد من حذف هذا السجل نهائياً من قاعدة البيانات وغرفة العمليات؟',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () => Navigator.pop(ctx, true),
            child: const Text('نعم، حذف نهائي 🗑️',
                style: TextStyle(fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    ...
    try {
      ...
      // تحديث القائمة المحلية داخل المانجر أيضاً
      AppStateManager().paymentAudits.removeWhere((p) => p.id == txId);
      AppStateManager().notifyListeners();

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ تم حذف السجل نهائياً من غرفة العمليات والسيرفر!'),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          centerTitle: true,
          title: const Text(
            'سجل المشتركين وتدقيق الإيصالات 🛡️',
            style: TextStyle(
                color: Color(0xFFD4AF37),
                fontSize: 15,
                fontWeight: FontWeight.bold),
          ...
          bottom: TabBar(
            ...
            tabs: const [
              Tab(
                ...
                text: 'سجل الاشتراكات والإيصالات 💎',
              ),
              Tab(
                icon: Icon(Icons.apartment_rounded, size: 20),
                text: 'توثيق المكاتب والمعارض 🏢',
              ),
            ],
          ),
        ),
        body: TabBarView(
          controller: _tabController,
          children: [
            // 🌟 التبويب 1: سجل المشتركين الدائم تحت المراقبة مع أزرار المكافأة والحذف
            _buildSubscriptionsMonitoringTab(),

            // 🏢 التبويب 2: توثيق المكاتب العقارية ومعارض السيارات
            _buildAgenciesTab(),
          ],
        ),
      ),
  ...
  // 🌟 واجهة مراقبة المشتركين والإيصالات (Live Realtime مع أزرار التحكم)
  Widget _buildSubscriptionsMonitoringTab() {
      ...
      builder: (context, snapshot) {
          ...
          return const Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 12),
                Text(
                  'لا توجد اشتراكات أو إيصالات حالياً 👍',
                  style: TextStyle(
                      color: Colors.white70,
                      fontSize: 14,
                      fontWeight: FontWeight.bold),
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final userName = item['user_name'] ?? 'مستخدم';
            final userPhone = item['user_phone'] ?? '';
            final planName = item['plan_name'] ?? 'باقة خاصة';
            final amountUsd = item['amount_usd'] ?? 0;
            final gateway = item['gateway'] ?? 'شام كاش / بينانس';
            final txRef = item['transaction_ref'] ??
                item['transaction_ref_or_tx_id'] ??
                '';
            final status = item['status'] ?? 'pending';
            final receiptUrl = item['receipt_image_url'];
            final durationLabel = item['duration_label'] ?? 'شهري (30 يوم)';
            ...
            return Card(
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // رأس البطاقة: اسم المشترك وحالته
                    Row(
                      children: [
                        CircleAvatar(
                          backgroundColor: const Color(0xFF0F172A),
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color: isApproved
                                  ? Colors.green
                                  : (isRejected ? Colors.red : Colors.amber),
                              width: 0.8,
                            ),
                          ),
                          child: Text(
                            isApproved
                                ? 'اشتراك نشط ومفعل ✓'
                                : (isRejected ? 'مرفوض ✕' : 'بانتظار التحقق ⏳'),
                            style: TextStyle(
                              color: isApproved
                                  ? Colors.greenAccent
                                  : (isRejected
                    ...
                    // تفاصيل الدفع والتواصل
                    Text('📞 هاتف المشترك: $userPhone',
                        style: const TextStyle(
                            color: Color(0xFF38BDF8), fontSize: 12)),
                    const SizedBox(height: 3),
                    Text('💰 المبلغ المدفوع: \$$amountUsd • بوابة: $gateway',
                        style: const TextStyle(
                            color: Colors.white70, fontSize: 12)),
                    if (txRef.isNotEmpty) ...[
                      const SizedBox(height: 3),
                      Text('🔢 رقم الحوالة / العملية: $txRef',
                          style: const TextStyle(
                              color: Colors.white60, fontSize: 11)),
                    ],
                    const SizedBox(height: 10),

                    // زر معاينة صورة الإيصال إن وُجدت
                    if (receiptUrl != null &&
                      ...
                      OutlinedButton.icon(
                        ...
                        onPressed: () {
                          showDialog(
                            ...
                            builder: (c) => Dialog(
                              ...
                              child: ClipRRect(
                                borderRadius: BorderRadius.circular(12),
                                child: Image.network(receiptUrl.toString(),
                                    fit: BoxFit.contain),
                              ),
                            ),
                          );
                        ...
                        icon: const Icon(Icons.receipt_long_rounded,
                            color: Color(0xFF38BDF8), size: 16),
                        label: const Text('معاينة صورة إشعار التحويل 🧾',
                            style: TextStyle(
                                color: Color(0xFF38BDF8), fontSize: 12)),
                      ),
                      const SizedBox(height: 12),
                    ...
                    // أزرار اتخاذ القرار الأساسي (في حال كان معلقاً)
                    if (!isApproved) ...[
                      Row(
                        children: [
                          Expanded(
                            child: ElevatedButton.icon(
                              ...
                              icon: const Icon(Icons.check_circle_rounded,
                                  size: 16),
                              label: const Text('موافقة وتفعيل فوري ✅',
                                  style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 12)),
                              onPressed: () {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
                                        '✅ تم تفعيل باقة المشترك ($userName) بالسيرفر!'),
                                    backgroundColor: const Color(0xFF16A34A),
                                  ),
                                );
                              },
                          ...
                          ElevatedButton.icon(
                            ...
                            label: const Text('رفض',
                                style: TextStyle(fontSize: 12)),
                            onPressed: () {
                              AppStateManager().rejectPaymentTransaction(
                                  txId, 'بيانات التحويل غير مطابقة');
                            },
                          ),
                        ],
                      ),
                    ...
                    // 🌟 لوحة أزرار التحكم الدائمة تحت المراقبة (المكافآت + التمديد + الحذف)
                    Container(
                      padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(
                        color: const Color(0xFF0F172A),
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('لوحة التحكم بالمشترك والمكافآت ⚙️:',
                              style: TextStyle(
                                  color: Color(0xFFD4AF37),
                                  fontSize: 11,
                                  fontWeight: FontWeight.bold)),
                          ...
                          Wrap(
                            ...
                            children: [
                              ...
                              ActionChip(
                                ...
                                avatar: const Icon(Icons.card_giftcard_rounded,
                                    color: Color(0xFFD4AF37), size: 14),
                                label: const Text('مكافأة +3 أيام 🎁',
                                    style: TextStyle(
                                        color: Colors.white, fontSize: 11)),
                                onPressed: () => _addBonusDays(item, 3),
                              ),
                              // زر مكافأة +7 أيام
                              ActionChip(
                                backgroundColor: const Color(0xFF1E293B),
                                ...
                                label: const Text('+7 أيام ⭐',
                                    style: TextStyle(
                                        color: Colors.white, fontSize: 11)),
                                onPressed: () => _addBonusDays(item, 7),
                              ),
                              // زر تجديد شهر كامل
                              ActionChip(
                                backgroundColor: const Color(0xFF1E293B),
                                ...
                                avatar: const Icon(Icons.autorenew_rounded,
                                    color: Color(0xFF38BDF8), size: 14),
                                label: const Text('تجديد شهر (30 يوم) 🔄',
                                    style: TextStyle(
                                        color: Colors.white, fontSize: 11)),
                                onPressed: () => _renewSubscription(item),
                              ),
                              // زر الحذف النهائي
                              ActionChip(
                                ...
                                label: const Text('حذف الاشتراك 🗑️',
                                    style: TextStyle(
                                        color: Colors.redAccent,
                                        fontSize: 11,
                                        fontWeight: FontWeight.bold)),
                                onPressed: () => _deleteSubscription(txId),
                              ),
                            ],
                          ),
                        ],
  ...
  Widget _buildAgenciesTab() {
    ...
    if (_pendingAgencies.isEmpty) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.apartment_rounded, size: 54, color: Colors.white24),
            SizedBox(height: 12),
            Text(
              'لا توجد طلبات توثيق مكاتب حالياً 👍',
              style: TextStyle(
                  color: Colors.white70,
                  fontSize: 14,
                  fontWeight: FontWeight.bold),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final item = _pendingAgencies[index];
        final id = item['id'];
        final name = item['agency_name'] ?? '';
        final cat = item['category'] ?? '';
        final city = item['city'] ?? '';
        final phone = item['phone'] ?? '';
        final payMethod = item['payment_method'] ?? 'غير محدد';
        final receiptUrl = item['payment_receipt_url'];
        ...
        return Card(
          ...
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(14),
            side: BorderSide(
                color: isApproved ? Colors.green : const Color(0xFFD4AF37),
                width: 1),
          ),
          child: Padding(
            padding: const EdgeInsets.all(14),
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                          isApproved ? 'معتمد وموثق ✓' : 'بانتظار التحقق',
                          style: TextStyle(
                ...
                const Divider(color: Colors.white12, height: 20),
                Text('📞 هاتف التواصل: $phone',
                    style: const TextStyle(
                        color: Color(0xFF38BDF8), fontSize: 12)),
                const SizedBox(height: 4),
                Text('💳 طريقة التحويل: $payMethod',
                    style:
                        const TextStyle(color: Colors.white70, fontSize: 12)),
                const SizedBox(height: 12),

                // زر معاينة الوصل المالي إن وُجد
                if (receiptUrl != null && receiptUrl.toString().isNotEmpty) ...[
                  OutlinedButton.icon(
                    ...
                    label: const Text('معاينة صورة إشعار التحويل 🧾',
                        style:
                            TextStyle(color: Color(0xFF38BDF8), fontSize: 12)),
                  ),
                  const SizedBox(height: 12),
                ],
                    ...
                    if (!isApproved)
                      Expanded(
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () => _approveAgency(id, name),
                          icon: const Icon(Icons.check_circle_rounded,
                              color: Colors.white, size: 16),
                          label: const Text('اعتماد وتفعيل الغرفة ✅',
                              style: TextStyle(
                                  color: Colors.white,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 12)),
...
class _LuxurySplashScreenState extends State<LuxurySplashScreen>
  ...
  void initState() {
    ...
    // الانتقال التلقائي السلس إلى لوحة التحكم الرئيسية بعد انتهاء الحركة
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        fit: StackFit.expand,
        children: [
          // تدرج لوني ملكي خلفي مع هالة ذهبية في المنتصف
          Container(
            decoration: const BoxDecoration(
          ...
          Center(
            child: FadeTransition(
              ...
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  // 1. عربة التسوق الذهبية المتحركة بانسيابية
                  SlideTransition(
                  ...
                  // 2. الشعار الفاخر بدقة عالية ونعومة بصرية
                  ScaleTransition(
                    ...
                    child: Column(
                      children: [
                        ...
                        Container(
                          ...
                          child: const Text(
                            'المنصة العقارية والتجارية الأولى في سوريا 2028',
                            style: TextStyle(
                              color: Color(0xFFD4AF37),
                              fontSize: 11,
                              fontWeight: FontWeight.bold,
                  ...
                  // 3. مؤشر التحميل الذهبي فائق النعومة
                  const SizedBox(
                    width: 24,
                    height: 24,
                    child: CircularProgressIndicator(
          ...
          const Positioned(
            ...
            child: Center(
              child: Text(
                'الإصدار المعتمد • فخر الصناعة البرمجية الوطنية',
                style: TextStyle(
...
class SupabaseAuthScreen extends StatefulWidget {
  ...
  const SupabaseAuthScreen({super.key, this.onAuthSuccess});
...
class _SupabaseAuthScreenState extends State<SupabaseAuthScreen>
  ...
  String _selectedGovernorate = 'دمشق';
  String _accountType = 'مستخدم عادي';
  ...
  // المصادقة الحقيقية عبر Supabase
  Future<void> _handleAuth() async {
    ...
    if (_isSignUp &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('كلمات المرور غير متطابقة! يرجى التأكد منها.'),
          backgroundColor: Colors.redAccent,
        ),
      );
      return;
    ...
    try {
      ...
      if (_isSignUp) {
        ...
        if (user != null) {
          // حفظ الملف في جدول profiles
          try {
            await supabase.from('profiles').upsert({
              'id': user.id,
              'full_name': _fullNameController.text.trim(),
              'phone_number': '+963 ${_phoneController.text.trim()}',
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('🎉 تم إنشاء حسابك بنجاح! تم تسجيل دخولك.'),
        ...
        final user = authRes.user;
        if (user != null) {
          String userName = user.email?.split('@').first ?? 'مستخدم سوق سوريا';
          String userPhone = '';
          try {
          ...
          // تحديث الجلسة وحفظ بيانات الدخول فوراً
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('مرحباً بك مجدداً يا $userName 👋'),
                backgroundColor: const Color(0xFF22C55E),
              ),
            );
            widget.onAuthSuccess?.call();
            // الانتقال المباشر والفوري إلى الشاشة الرئيسية
      ...
      if (mounted) {
        ...
        if (errorMsg.contains('Invalid login credentials')) {
          ...
          errorMsg = 'هذا الحساب مسجل مسبقاً، يرجى تسجيل الدخول';
        }
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _handleForgotPassword() async {
    ...
    if (email.isEmpty || !email.contains('@')) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'يرجى كتابة بريدك الإلكتروني في الحقل أولاً لاسترجاع كلمة المرور'),
          backgroundColor: Colors.orange,
        ),
      );
      return;
    ...
    try {
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: const Row(
              children: [
                Icon(Icons.mark_email_read_rounded, color: Color(0xFFD4AF37)),
                SizedBox(width: 8),
                Text('رابط استرجاع كلمة المرور',
                    style: TextStyle(color: Colors.white, fontSize: 15)),
              ],
            ),
            content: Text(
              'أرسلنا لك رابط إعادة تعيين كلمة المرور إلى:\n$email\nيرجى مراجعة بريدك الإلكتروني.',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('تعذر إرسال الرابط: $e'),
              backgroundColor: Colors.redAccent),
        );
      }
    }
                ...
                children: [
                  ...
                  Center(
                    child: Container(
                      ...
                      child: const Center(
                        child: SyrianIndependenceFlag(width: 38, height: 24),
                      ),
                    ),
                  ),
                  const SizedBox(height: 14),
                  ...
                  const SizedBox(height: 4),
                  const Text(
                    'المنصة الوطنية الأولى للتجارة والعقارات والسيارات 💚🤍🖤',
                    textAlign: TextAlign.center,
                    style: TextStyle(color: Colors.white54, fontSize: 11),
                  ),
                  const SizedBox(height: 18),

                  // أزرار التبديل الأنيقة
                  Container(
                          ...
                          child: GestureDetector(
                            ...
                            child: AnimatedContainer(
                              ...
                              child: Text(
                                ...
                                style: TextStyle(
                                  color:
                                      _isSignUp ? Colors.black : Colors.white60,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 12.5,
                                ),
                              ),
                      ...
                      children: [
                            ...
                            children: [
                              ...
                              DropdownButton<String>(
                                ...
                                items: const [
                                  DropdownMenuItem(
                                      value: 'مستخدم عادي',
                                      child: Text('👤 بائع / مستخدم')),
                                  DropdownMenuItem(
                                      value: 'مكتب عقاري',
                                      child: Text('🏢 مكتب عقاري')),
                                  DropdownMenuItem(
                                      value: 'معرض سيارات',
                                      child: Text('🚗 معرض سيارات')),
                                ],
                                onChanged: (val) {
                                  if (val != null)
                        ...
                        TextFormField(
                          ...
                          decoration: _buildInputDecoration(
                            label: 'الاسم الكامل أو اسم المنشأة',
                            icon: Icons.person_outline_rounded,
                          ),
                          validator: (v) =>
                              _isSignUp && (v == null || v.trim().length < 3)
                                  ? 'يرجى إدخال اسم صحيح'
                                  : null,
                        ),
                        const SizedBox(height: 10),

                        // رقم الواتساب مع علم الاستقلال
                        TextFormField(
                          controller: _phoneController,
                          keyboardType: TextInputType.phone,
                          style: const TextStyle(
                              color: Colors.white, fontSize: 12.5),
                          decoration: InputDecoration(
                            labelText: 'رقم الواتساب (مثال: 944123456)',
                          ...
                          validator: (v) =>
                              _isSignUp && (v == null || v.trim().length < 8)
                                  ? 'يرجى إدخال رقم صحيح'
                                  : null,
                        ),
                        const SizedBox(height: 10),
                  ...
                  // تأكيد كلمة المرور (في إنشاء الحساب فقط)
                  if (_isSignUp) ...[
                    const SizedBox(height: 10),
                    TextFormField(
                  ...
                  // زر نسيت كلمة المرور (عند تسجيل الدخول)
                  if (!_isSignUp) ...[
                    Align(
                      alignment: Alignment.centerLeft,
                      child: TextButton(
                        ...
                        child: const Text(
                          'نسيت كلمة المرور؟ اضغط هنا للاسترجاع',
                          style:
  ...
  InputDecoration _buildInputDecoration({
    ...
    return InputDecoration(
      labelText: label,
      labelStyle: const TextStyle(color: Colors.white54, fontSize: 11.5),
...
// ==============================================================================
// 💎 منظومة الباقات والاشتراكات VIP وإدارة غرفة العمليات بالدفع المباشر
// ==============================================================================

// بيانات الدفع الثابتة للمنصة
const String kPlatformShamCashAccount = '0308a7227251b7c8ebca471cd30b15a8';
const String kPlatformBinanceWallet = 'TCHJ8QyEijnRsQmyXJWBCoiuPET1mZqBK2';
const String kPlatformAdminPhone = '948526684';

// ------------------------------------------------------------------------------
// 1. شاشة استعراض الباقات والاشتراك للمستخدم مع خيارات الدفع
// ------------------------------------------------------------------------------
...
class _VipPackagesScreenState extends State<VipPackagesScreen> {
  ...
  void _openSubscriptionCheckout(Map<String, dynamic> pkg) {
    final user = Supabase.instance.client.auth.currentUser;
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('يرجى تسجيل الدخول أولاً للاشتراك في الباقة'),
          backgroundColor: Colors.orange,
        ),
      );
      Navigator.push(
        context,
        MaterialPageRoute(builder: (ctx) => const SupabaseAuthScreen()),
      );
      return;
    }
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (ctx) => SubscriptionCheckoutScreen(package: pkg),
      ),
    );
  }
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.workspace_premium_rounded, color: Color(0xFFD4AF37)),
              SizedBox(width: 8),
              Text('باقات الترقية والاشتراكات VIP 💎',
                  ...
                  Container(
                    ...
                    child: const Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'اختر الباقة المناسبة لأعمالك، وضاعف مبيعاتك مع أولوية الظهور والإعلانات المميزة.',
                            style:
  ...
  Widget _buildPackageCard(Map<String, dynamic> pkg) {
    ...
    final colorHex = pkg['badge_color'] as String? ?? '#D4AF37';
    final badgeColor = Color(int.parse(colorHex.replaceFirst('#', '0xFF')));
            ...
            child: Column(
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () => _openSubscriptionCheckout(pkg),
                  child: const Center(
                    child: Text(
                      'طلب الاشتراك والدفع الآن 💳',
                      style: TextStyle(
                          color: Colors.black,
                          fontWeight: FontWeight.bold,
                          fontSize: 13),
...
// ------------------------------------------------------------------------------
// 2. استمارة رفع إشعار الدفع عبر شام كاش أو بينانس
// ------------------------------------------------------------------------------
class SubscriptionCheckoutScreen extends StatefulWidget {
  final Map<String, dynamic> package;
  const SubscriptionCheckoutScreen({super.key, required this.package});
...
class _SubscriptionCheckoutScreenState
  ...
  // اختيار ورفع إيصال الدفع إلى الـ Bucket في Supabase
  Future<void> _pickAndUploadReceipt() async {
    final picker = ImagePicker();
    final image =
    ...
    try {
      ...
      setState(() {
        _uploadedReceiptUrl = publicUrl;
      });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('تم رفع إيصال التحويل بنجاح!'),
              backgroundColor: Color(0xFF22C55E)),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('فشل رفع الإيصال: $e'),
              backgroundColor: Colors.redAccent),
        );
      }
    } finally {
  ...
  Future<void> _submitRequest() async {
    if (_uploadedReceiptUrl == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('يرجى رفع صورة إيصال التحويل أولاً'),
            backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    try {
      ...
      await Supabase.instance.client.from('subscription_requests').insert({
        ...
        'payment_method': _paymentMethod,
        'receipt_image_url': _uploadedReceiptUrl,
        'duration_days': widget.package['duration_days'],
        'status': 'pending',
      });
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: const Text(
              'وصل طلبك لغرفة العمليات المركزية. سيتم تدقيق الإشعار وتفعيل باقتك خلال دقائق معدودة مع إرسال إشعار لك.',
              style: TextStyle(color: Colors.white70, fontSize: 13),
            ),
            actions: [
              TextButton(
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: const Color(0xFF0F172A),
          title: Text('دفع اشتراك: ${widget.package['name']}',
              style: const TextStyle(fontSize: 14)),
        ),
        body: SingleChildScrollView(
          padding: const EdgeInsets.all(16),
          child: Column(
            ...
            children: [
              // اختيار وسيلة الدفع
              const Text('اختر وسيلة الدفع المناسبة لك:',
                  style: TextStyle(
                      color: Colors.white,
                      fontWeight: FontWeight.bold,
                      fontSize: 13)),
              ...
              Row(
                children: [
                  Expanded(
                    child: GestureDetector(
                      onTap: () => setState(() => _paymentMethod = 'شام كاش'),
                      child: Container(
                        padding: const EdgeInsets.symmetric(vertical: 12),
                        decoration: BoxDecoration(
                          color: _paymentMethod == 'شام كاش'
                  ...
                  Expanded(
                    child: GestureDetector(
                      onTap: () =>
                          setState(() => _paymentMethod = 'بايننس USDT'),
                      child: Container(
                        padding: const EdgeInsets.symmetric(vertical: 12),
                        decoration: BoxDecoration(
                          color: _paymentMethod == 'بايننس USDT'
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      _paymentMethod == 'شام كاش'
                          ? 'بيانات حساب شام كاش المعتمد:'
                          : 'عنوان محفظة بايننس (TRC20):',
                      style:
                          const TextStyle(color: Colors.white70, fontSize: 12),
                    ),
                    const SizedBox(height: 8),
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: Text(
                              _paymentMethod == 'شام كاش'
                                  ? kPlatformShamCashAccount
                                  : kPlatformBinanceWallet,
                              style: const TextStyle(
                                  color: Color(0xFFD4AF37),
                                  fontSize: 11,
                                  fontFamily: 'monospace'),
                    ...
                    const SizedBox(height: 6),
                    Text(
                      'المبلغ المطلوب تحويله: ${_paymentMethod == 'شام كاش' ? '${widget.package['price_syp']} ل.س' : '${widget.package['price_usdt']} USDT'}',
                      style: const TextStyle(
                          color: Color(0xFF22C55E),
                          fontSize: 12,
                          fontWeight: FontWeight.bold),
              ...
              TextFormField(
                ...
                style: const TextStyle(color: Colors.white, fontSize: 13),
                decoration: InputDecoration(
                  labelText: 'اسم صاحب الحساب / المنشأة',
                  labelStyle:
                      const TextStyle(color: Colors.white54, fontSize: 12),
                  filled: true,
                  fillColor: const Color(0xFF1E293B),
              ...
              TextFormField(
                ...
                style: const TextStyle(color: Colors.white, fontSize: 13),
                decoration: InputDecoration(
                  labelText: 'رقم الهاتف للتواصل وتأكيد التفعيل',
                  labelStyle:
                      const TextStyle(color: Colors.white54, fontSize: 12),
                  filled: true,
                  fillColor: const Color(0xFF1E293B),
                        ...
                        : _uploadedReceiptUrl != null
                            ? const Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 6),
                                  Text(
                                      'تم إرفاق إيصال التحويل بنجاح (اضغط للتغيير)',
                                      style: TextStyle(
                                          color: Colors.white70, fontSize: 11)),
                                ],
                              )
                            : const Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 6),
                                  Text(
                                      'اضغط هنا لرفع صورة إشعار أو سكرين التحويل 📷',
                                      style: TextStyle(
                                          color: Colors.white70, fontSize: 11)),
                                ],
                              ),
              ...
              ElevatedButton(
                ...
                child: _isSubmitting
                    ...
                    : const Text('إرسال إشعار الدفع لتفعيل الباقة فوراً 🚀',
                        style: TextStyle(
                            color: Colors.black,
                            fontWeight: FontWeight.bold,
                            fontSize: 13)),
...
// ------------------------------------------------------------------------------
// 3. جناح إدارة الباقات والطلبات في غرفة العمليات (خاص بـ عبده عواد وسامر عواد)
// ------------------------------------------------------------------------------
...
// عارض صور الإيصالات بملء الشاشة مع إمكانية التكبير والتصغير الفائق حتى 5 أضعاف
class FullScreenReceiptViewer extends StatelessWidget {
  final String imageUrl;
  final String title;

  const FullScreenReceiptViewer({
    super.key,
    required this.imageUrl,
    required this.title,
  });
        ...
        child: InteractiveViewer(
          ...
          child: Image.network(
            ...
            errorBuilder: (_, __, ___) => const Center(
              child: Text('تعذر تحميل الصورة الأصلية للإيصال',
                  style: TextStyle(color: Colors.white54)),
            ),
          ),
        ),
...
// قائمة تدقيق الطلبات والموافقة عليها مع شريط بحث وعد تنازلي وتجديد فوري
class _AdminSubscriptionRequestsList extends StatefulWidget {
  ...
  State<_AdminSubscriptionRequestsList> createState() =>
      _AdminSubscriptionRequestsListState();
}

class _AdminSubscriptionRequestsListState
    extends State<_AdminSubscriptionRequestsList> {
  final TextEditingController _searchController = TextEditingController();
  String _searchQuery = '';

  // تفعيل الباقة وتحديث حساب المستخدم في جدول profiles لحساب العد التنازلي الحقيقي
  Future<void> _approveRequest(Map<String, dynamic> req,
    ...
    try {
      ...
      // 2. تحديث بروفايل المستخدم الحقيقي ليتفعل له العداد فوراً في حسابه
  ...
  // نافذة تجديد وتمديد الاشتراك بعدد أيام يحدده المسؤول
  void _showRenewDialog(Map<String, dynamic> req) {
    ...
    showDialog(
      ...
      builder: (ctx) => Directionality(
        ...
        child: AlertDialog(
          ...
          shape:
              RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
          title: Text('تجديد / تمديد اشتراك ${req['user_name']}',
              style: const TextStyle(color: Colors.white, fontSize: 14)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('حدد عدد الأيام الإضافية التي ترغب بمنحها للمشترك:',
                  style: TextStyle(color: Colors.white70, fontSize: 12)),
              const SizedBox(height: 12),
              TextField(
                controller: daysCtrl,
  ...
  String _calculateRemainingTime(String? expiresAtStr) {
    if (expiresAtStr == null) return 'غير محدد';
    final expiresAt = DateTime.tryParse(expiresAtStr);
    if (expiresAt == null) return 'غير محدد';

    final diff = expiresAt.difference(DateTime.now());
    if (diff.isNegative) return 'منتهي الصلاحية ⚠️';
    return '${diff.inDays} يوم و ${diff.inHours % 24} ساعة متبقية ⏳';
  }
      ...
      children: [
        ...
        Padding(
          ...
          child: TextField(
            ...
            style: const TextStyle(color: Colors.white, fontSize: 13),
            decoration: InputDecoration(
              hintText: 'ابحث بالاسم، برقم الهاتف، أو بنوع الباقة...',
              hintStyle: const TextStyle(color: Colors.white38, fontSize: 12),
              prefixIcon:
                  const Icon(Icons.search_rounded, color: Color(0xFFD4AF37)),
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (ctx, idx) {
                  ...
                  final remaining = _calculateRemainingTime(r['expires_at']);
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 6),
                          Text(
                              'الباقة: ${r['package_name']} • الدفع عبر: ${r['payment_method']}',
                              style: const TextStyle(
                                  color: Color(0xFFD4AF37), fontSize: 12)),
                          Text('رقم الهاتف: ${r['user_phone']}',
                              style: const TextStyle(
                                  color: Colors.white70, fontSize: 11.5)),
                          if (isApproved) ...[
                            const SizedBox(height: 6),
                          ...
                          // معاينة الإيصال مع إمكانية التكبير ملء الشاشة حتى 5 أضعاف
                            ...
                            GestureDetector(
                              onTap: () {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (ctx) => FullScreenReceiptViewer(
                                      imageUrl: r['receipt_image_url'],
                                      title:
                                          'إيصال دفع: ${r['user_name']} (${r['package_name']})',
                                    ),
                                  ),
                                );
                              },
                                    ...
                                    child: const Center(
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          SizedBox(width: 4),
                                          Text(
                                              'اضغط لتكبير وقراءة الإيصال بملء الشاشة 🔍',
                                              style: TextStyle(
                                                  color: Colors.white,
                                                  fontSize: 10)),
                                        ],
                                  ...
                                  child: ElevatedButton.icon(
                                    ...
                                    style: ElevatedButton.styleFrom(
                                        ...
                                        shape: RoundedRectangleBorder(
                                            borderRadius:
                                                BorderRadius.circular(8))),
                                    onPressed: () => _showRenewDialog(r),
                                    label: const Text(
                                        'تجديد أو تمديد الاشتراك ⏱️',
                                        style: TextStyle(
                                            color: Colors.black,
                                            fontWeight: FontWeight.bold,
                                            fontSize: 12)),
...
class _AdminPackagesEditorList extends StatefulWidget {
  ...
  @override
  State<_AdminPackagesEditorList> createState() =>
      _AdminPackagesEditorListState();
}
                        ...
                        children: [
                          ...
                          IconButton(
                            icon: const Icon(Icons.delete_outline,
                                color: Colors.redAccent, size: 18),
                            onPressed: () =>
                                setModalState(() => features.removeAt(index)),
                          ),
                        ],
                      ),
                    );
                    ...
                    children: [
                      Expanded(
                        child: TextField(
                          ...
                          style: const TextStyle(
                              color: Colors.white, fontSize: 12),
                          decoration: const InputDecoration(
                            hintText: 'اكتب ميزة جديدة ثم اضغط إضافة...',
                            hintStyle:
                      ...
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(
                            backgroundColor: const Color(0xFFD4AF37)),
                        onPressed: () {
                          if (newFeatureCtrl.text.trim().isNotEmpty) {
                            setModalState(() {
                              features.add(newFeatureCtrl.text.trim());
                              newFeatureCtrl.clear();
                            });
                          }
                        },
                    ...
                    onPressed: () async {
                      ...
                      if (mounted) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: Text(isNew
                                  ? '✅ تم نشر الباقة الجديدة في المنصة بنجاح!'
                                  : '✅ تم حفظ وتحديث الباقة في السيرفر!'),
                              backgroundColor: const Color(0xFF22C55E)),
                        );
                      }
                    },
...
// ==============================================================================
// 👑 شاشة الملف الشخصي الملكية المعتمدة (UserProfileScreen)
// ==============================================================================
class UserProfileScreen extends StatefulWidget {
  final String? targetUserId;
  final String? targetUserName;

  const UserProfileScreen({super.key, this.targetUserId, this.targetUserName});
...
class _UserProfileScreenState extends State<UserProfileScreen> {
  ...
  void _initUser() {
    ...
    if (widget.targetUserId != null) {
      ...
      _isOwnProfile = (authUser != null && authUser.id == _currentUserId);
    ...
    _loadLocalCacheAndFetch();
  }

  Future<void> _loadLocalCacheAndFetch() async {
    if (_isOwnProfile) {
      try {
        final prefs = await SharedPreferences.getInstance();
        final localName = prefs.getString('ss_user_name');
        final localPhone = prefs.getString('ss_user_phone');
        final localAvatar = prefs.getString('ss_user_avatar');
        final localBio = prefs.getString('ss_user_bio');
        final localGov = prefs.getString('ss_user_gov');
        final localFb = prefs.getString('ss_user_fb');
        final localTg = prefs.getString('ss_user_tg');
        final localIg = prefs.getString('ss_user_ig');
        final localYt = prefs.getString('ss_user_yt');
        final localTt = prefs.getString('ss_user_tt');

        if (localName != null || localAvatar != null) {
          setState(() {
            _profileData = {
              'full_name': localName ?? AppStateManager().currentUserName,
              'phone_number': localPhone ?? AppStateManager().currentUserPhone,
              'avatar_url': localAvatar,
              'bio': localBio ?? 'تاجر ومستخدم معتمد في سوق سوريا الشامل.',
              'governorate': localGov ?? 'دمشق',
              'location_address': localGov ?? 'دمشق',
              'facebook_url': localFb ?? '',
              'telegram_url': localTg ?? '',
              'instagram_url': localIg ?? '',
              'youtube_url': localYt ?? '',
              'tiktok_url': localTt ?? '',
            };
          });
  ...
  Future<void> _fetchProfileAndStats() async {
    ...
    try {
      ...
      int views = (data?['profile_views'] as int? ?? 0);
      if (!_isOwnProfile) {
        views += 1;
        try {
          await Supabase.instance.client
              .from('profiles')
              .update({'profile_views': views}).eq('id', _currentUserId);
        } catch (_) {}
      }
      ...
      try {
        final adsRes = await Supabase.instance.client
            .from('ads')
            .select('id, seller_positive_likes, seller_dislikes')
            .eq('user_id', _currentUserId);

        final adsList = adsRes as List? ?? [];
        activeCount = adsList.length;
        for (var ad in adsList) {
          posCount += (ad['seller_positive_likes'] as int? ?? 0);
          negCount += (ad['seller_dislikes'] as int? ?? 0);
        }
      } catch (_) {}

      int days = 30;
      if (data?['vip_expires_at'] != null) {
        try {
          final exp = DateTime.parse(data!['vip_expires_at']);
      ...
      if (mounted) {
        setState(() {
          ...
          _profileData!['profile_views'] = views;
          _activeAdsCount = activeCount;
          _totalPositiveLikes = posCount > 0
              ? posCount
              : (_profileData?['positive_ratings'] ?? 5);
          _totalDislikes = negCount;
          _daysRemaining = days;
          _isLoading = false;
        });
      }
  ...
  void _openEditProfileDialog() {
    ...
    final locationCtrl = TextEditingController(
        text: _profileData!['location_address'] ??
            _profileData!['governorate'] ??
            'دمشق');
    final fbCtrl =
        TextEditingController(text: _profileData!['facebook_url'] ?? '');
    final tgCtrl =
        TextEditingController(text: _profileData!['telegram_url'] ?? '');
    final igCtrl =
        TextEditingController(text: _profileData!['instagram_url'] ?? '');
    final ytCtrl =
        TextEditingController(text: _profileData!['youtube_url'] ?? '');
    final ttCtrl =
        TextEditingController(text: _profileData!['tiktok_url'] ?? '');
    String? currentAvatar = _profileData!['avatar_url'];
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('تعديل الملف الشخصي والمعلومات ✏️',
                          style: TextStyle(
                              color: Colors.white,
                              fontWeight: FontWeight.bold,
                              fontSize: 15)),
                        ...
                        Positioned(
                          ...
                          child: InkWell(
                            onTap: () async {
                              ...
                              setModalState(() => isUploadingImg = true);
                              try {
                                final bytes = await img.readAsBytes();
                                final uploadedUrl =
                                    await StorageUploadService.uploadImageBytes(
                                  bucketName: kStorageBucketFeedbacks,
                                  imageBytes: bytes,
                                  prefix: 'avatar',
                                );
                                if (uploadedUrl != null &&
                                  ...
                                  setModalState(() => isUploadingImg = false);
                                }
                              } catch (_) {
                                setModalState(() => isUploadingImg = false);
                              }
                  ...
                  const SizedBox(height: 14),
                  _buildInput(nameCtrl, 'الاسم الكامل أو اسم المنشأة',
                      Icons.person_outline),
                  const SizedBox(height: 10),
                  _buildInput(phoneCtrl, 'رقم الواتساب والهاتف المعتمد',
                      Icons.phone_android),
                  const SizedBox(height: 10),
                  _buildInput(locationCtrl, 'المحافظة / العنوان الدقيق',
                      Icons.location_on_outlined),
                  const SizedBox(height: 10),
                  _buildInput(bioCtrl, 'لمحة ونبذة عنك وعن نشاطك التجاري',
                      Icons.info_outline,
                      maxLines: 2),
                  const SizedBox(height: 12),
                  const Text('روابط التواصل والحسابات (ستفتح للمستخدم مباشرة):',
                      style: TextStyle(
                          color: Color(0xFFD4AF37),
                          fontSize: 11.5,
                          fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  _buildInput(fbCtrl, 'رابط صفحة فيسبوك', Icons.facebook),
                  const SizedBox(height: 8),
                  _buildInput(
                      tgCtrl, 'معرّف أو رابط تلغرام', Icons.send_rounded),
                  const SizedBox(height: 8),
                  _buildInput(
                      igCtrl, 'حساب إنستغرام', Icons.camera_alt_outlined),
                  const SizedBox(height: 8),
                  _buildInput(ytCtrl, 'رابط قناة يوتيوب',
                      Icons.video_collection_outlined),
                  const SizedBox(height: 8),
                  _buildInput(ttCtrl, 'حساب تيك توك', Icons.music_note_rounded),
                  const SizedBox(height: 18),
                  ElevatedButton(
                    style: ElevatedButton.styleFrom(
                      backgroundColor: const Color(0xFFD4AF37),
                    ...
                    onPressed: () async {
                      ...
                      final newFb = fbCtrl.text.trim();
                      final newTg = tgCtrl.text.trim();
                      final newIg = igCtrl.text.trim();
                      final newYt = ytCtrl.text.trim();
                      final newTt = ttCtrl.text.trim();

                      Navigator.pop(ctx);

                      // 1. تحديث مباشر وفوري لذاكرة الشاشة الحالية
                      setState(() {
                        if (_profileData == null) _profileData = {};
                        _profileData!['full_name'] = newName;
                        _profileData!['display_name'] = newName;
                        _profileData!['phone_number'] = newPhone;
                        _profileData!['location_address'] = newLocation;
                        _profileData!['governorate'] = newLocation;
                        _profileData!['bio'] = newBio;
                        _profileData!['avatar_url'] = currentAvatar;
                        _profileData!['facebook_url'] = newFb;
                        _profileData!['telegram_url'] = newTg;
                        _profileData!['instagram_url'] = newIg;
                        _profileData!['youtube_url'] = newYt;
                        _profileData!['tiktok_url'] = newTt;
                      });

                      // 2. تحديث الجلسة العامة للتطبيق
                      final user = Supabase.instance.client.auth.currentUser;
                      await AppStateManager().setSessionUser(
                        userId: user?.id ?? _currentUserId,
                        email:
                            user?.email ?? AppStateManager().currentUserEmail,
                        name: newName,
                        phone: newPhone,
                      );

                      // 3. التثبيت الدائم في ذاكرة الجهاز المشفرة SharedPreferences (حتى لا تختفي أبداً!)
                      try {
                        final prefs = await SharedPreferences.getInstance();
                        await prefs.setString('ss_user_name', newName);
                        await prefs.setString('ss_user_phone', newPhone);
                        await prefs.setString('ss_user_bio', newBio);
                        await prefs.setString('ss_user_gov', newLocation);
                        await prefs.setString('ss_user_fb', newFb);
                        await prefs.setString('ss_user_tg', newTg);
                        await prefs.setString('ss_user_ig', newIg);
                        await prefs.setString('ss_user_yt', newYt);
                        await prefs.setString('ss_user_tt', newTt);
                        if (currentAvatar != null) {
                          await prefs.setString(
                              'ss_user_avatar', currentAvatar!);
                        }
                      } catch (_) {}

                      // 4. الحفظ في Supabase بأمان تام
                      if (user != null) {
                        try {
                          await Supabase.instance.client
                              .from('profiles')
                            ...
                            'bio': newBio,
                            'governorate': newLocation,
                            'location_address': newLocation,
                            'facebook_url': newFb,
                            'telegram_url': newTg,
                            'instagram_url': newIg,
                            'youtube_url': newYt,
                          ...
                          debugPrint('Primary upsert notice: $e1');
                          try {
                            await Supabase.instance.client
                                .from('profiles')
                                .upsert({
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                                '✅ تم حفظ وتثبيت بياناتك وصورتك في الملف بنجاح دائم!'),
                            backgroundColor: Color(0xFF22C55E),
                          ),
                        );
                      }
  ...
  Widget build(BuildContext context) {
    if (_currentUserId == 'guest') {
      return Directionality(
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            backgroundColor: const Color(0xFF0F172A),
            title: const Text('الملف التعريفي والحساب',
                style: TextStyle(color: Colors.white, fontSize: 15)),
            centerTitle: true,
            leading: IconButton(
              icon: const Icon(Icons.arrow_back_ios_new_rounded,
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.person_outline_rounded,
                        size: 45, color: Color(0xFFD4AF37)),
                  ),
                  const SizedBox(height: 16),
                  const Text('أهلاً بك يا زائرنا الكريم 👋',
                      style: TextStyle(
                          color: Colors.white,
                          fontSize: 17,
                          fontWeight: FontWeight.bold)),
                  const SizedBox(height: 6),
                  const Text(
                    'سجّل حسابك للحصول على ملف موثق، ونشر إعلاناتك، ومتابعة نشاطك التجاري.',
                    textAlign: TextAlign.center,
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (ctx) => SupabaseAuthScreen(
                                onAuthSuccess: _fetchProfileAndStats)),
                      );
                    },
    ...
    final bool isCurrentUserMe = currentAuthUser != null &&
        (_currentUserId == currentAuthUser.id ||
            _currentUserId == manager.currentUserId);

    final pkgName = _profileData?['vip_package'] ?? 'مجاني';
    final bool isVip = pkgName != 'مجاني';
    final Color badgeColor =
        isVip ? const Color(0xFFD4AF37) : const Color(0xFF94A3B8);
    final String fullName = _profileData?['full_name'] ??
        _profileData?['display_name'] ??
        (isCurrentUserMe
            ? manager.currentUserName
            : (widget.targetUserName ?? 'مستخدم سوق سوريا'));
    final String phone = _profileData?['phone_number'] ??
        _profileData?['phone'] ??
        (isCurrentUserMe ? manager.currentUserPhone : '');
    final String location = _profileData?['location_address'] ??
        _profileData?['governorate'] ??
        'سوريا 💚🤍🖤';
    final String bio =
        _profileData?['bio'] ?? 'تاجر ومستخدم معتمد في سوق سوريا الشامل.';
    final int views = _profileData?['profile_views'] ?? 1;
    final String? avatarUrl = _profileData?['avatar_url'];

    final String fbUrl = _profileData?['facebook_url'] ?? '';
    final String tgUrl = _profileData?['telegram_url'] ?? '';
    final String igUrl = _profileData?['instagram_url'] ?? '';
    final String ytUrl = _profileData?['youtube_url'] ?? '';
    final String ttUrl = _profileData?['tiktok_url'] ?? '';
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        body: RefreshIndicator(
          onRefresh: _fetchProfileAndStats,
          color: const Color(0xFFD4AF37),
          child: ListView(
            padding: const EdgeInsets.all(16),
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: isVip
                        ...
                        : [const Color(0xFF182234), const Color(0xFF0F172A)],
                    begin: Alignment.topRight,
                    end: Alignment.bottomLeft,
                  ),
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                      color: badgeColor.withOpacity(0.6), width: 1.5),
                  boxShadow: [
                    BoxShadow(
                      color: badgeColor.withOpacity(0.15),
                      blurRadius: 20,
                ...
                child: Column(
                  children: [
                    ...
                    // 🌟 العد التنازلي الحي الذكي وزر تجديد الاشتراك التلقائي
                    if (isVip && _daysRemaining > 0) ...[
                      const SizedBox(height: 8),
                      Container(
                        padding: const EdgeInsets.symmetric(
                            horizontal: 12, vertical: 4),
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            const SizedBox(width: 6),
                            Text(
                              'باقي على انتهاء اشتراكك: $_daysRemaining يوماً ⏳',
                              style: const TextStyle(
                                  color: Color(0xFFD4AF37),
                                  fontSize: 11.5,
                                  fontWeight: FontWeight.bold),
                    ...
                    ] else if (isVip && _daysRemaining <= 0) ...[
                      const SizedBox(height: 8),
                      Container(
                        padding: const EdgeInsets.all(10),
                        decoration: BoxDecoration(
                          ...
                          children: [
                            const Row(
                              ...
                              children: [
                                ...
                                SizedBox(width: 6),
                                Text(
                                  'انتهت فترة اشتراكك في الباقة ⚠️',
                                  style: TextStyle(
                                      color: Colors.redAccent,
                                      fontSize: 12,
                                      fontWeight: FontWeight.bold),
              ...
              const SizedBox(height: 14),
              // زر منشوراتي مع فتح الإعلانات بالصفحة الكاملة
              ElevatedButton.icon(
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF1E293B),
                  foregroundColor: Colors.white,
                ...
                label: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('منشوراتي وإعلاناتي المعروضة بالسوق',
                        style: TextStyle(
                            fontWeight: FontWeight.bold, fontSize: 13)),
                    Container(
                      padding: const EdgeInsets.symmetric(
                      ...
                      decoration: BoxDecoration(
                          color: const Color(0xFFD4AF37),
                          borderRadius: BorderRadius.circular(8)),
                      child: Text('$_activeAdsCount إعلان',
                          style: const TextStyle(
                              color: Colors.black,
                              fontWeight: FontWeight.bold,
                              fontSize: 11)),
                ...
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (ctx) => UserAdsListScreen(
                          userId: _currentUserId, userName: fullName),
                    ),
                  );
                },
              ),
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('قنوات التواصل المعتمدة (اضغط للتواصل) 💬',
                        style: TextStyle(
                            color: Color(0xFFD4AF37),
                            fontWeight: FontWeight.bold,
                            fontSize: 12.5)),
                    ...
                    if (fbUrl.isNotEmpty)
                      ListTile(
                        ...
                        title: const Text('صفحة فيسبوك',
                            style:
                                TextStyle(color: Colors.white, fontSize: 12.5)),
                        trailing: const Icon(Icons.open_in_new,
                            color: Colors.white54, size: 16),
                        onTap: () async {
                          final u = fbUrl.startsWith('http')
                              ? fbUrl
                              : 'https://$fbUrl';
                          try {
                    ...
                    if (tgUrl.isNotEmpty)
                      ListTile(
                        ...
                        title: const Text('قناة / محادثة تلغرام',
                            style:
                                TextStyle(color: Colors.white, fontSize: 12.5)),
                        trailing: const Icon(Icons.open_in_new,
                            color: Colors.white54, size: 16),
                        onTap: () async {
                          final u = tgUrl.startsWith('http')
                              ? tgUrl
                              : 'https://$tgUrl';
                          try {
                            await launchUrl(Uri.parse(u),
                    ...
                    if (igUrl.isNotEmpty)
                      ListTile(
                        ...
                        title: const Text('حساب إنستغرام',
                            style:
                                TextStyle(color: Colors.white, fontSize: 12.5)),
                        trailing: const Icon(Icons.open_in_new,
                            color: Colors.white54, size: 16),
                        onTap: () async {
                          final u = igUrl.startsWith('http')
                              ? igUrl
                              : 'https://$igUrl';
                          try {
                            await launchUrl(Uri.parse(u),
                    ...
                    if (ytUrl.isNotEmpty)
                      ListTile(
                        ...
                        title: const Text('قناة يوتيوب',
                            style:
                                TextStyle(color: Colors.white, fontSize: 12.5)),
                        trailing: const Icon(Icons.open_in_new,
                            color: Colors.white54, size: 16),
                        onTap: () async {
                          final u = ytUrl.startsWith('http')
                              ? ytUrl
                              : 'https://$ytUrl';
                          try {
                            await launchUrl(Uri.parse(u),
                    ...
                    if (ttUrl.isNotEmpty)
                      ListTile(
                        ...
                        title: const Text('حساب تيك توك',
                            style:
                                TextStyle(color: Colors.white, fontSize: 12.5)),
                        trailing: const Icon(Icons.open_in_new,
                            color: Colors.white54, size: 16),
                        onTap: () async {
                          final u = ttUrl.startsWith('http')
                              ? ttUrl
                              : 'https://$ttUrl';
                          try {
                            await launchUrl(Uri.parse(u),
                    ...
                    if (phone.isEmpty &&
                      ...
                      const Padding(
                        padding: EdgeInsets.symmetric(vertical: 8),
                        child: Text(
                            'لم تتم إضافة وسائل تواصل بعد. اضغط زر التعديل لإضافتها ✏️',
                            style:
                                TextStyle(color: Colors.white54, fontSize: 11)),
                      ),
                  ],
...
// ==============================================================================
// 📢 شاشة منشوراتي الحية (تفتح صفحة الإعلان الكاملة FullAdDetailsScreen فوراً)
// ==============================================================================
class UserAdsListScreen extends StatelessWidget {
  final String userId;
  final String userName;

  const UserAdsListScreen(
      ...
      child: Scaffold(
        ...
        body: FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            if (adsData.isEmpty) {
              return const Center(
                child: Text('لا توجد إعلانات حية معروضة حالياً لهذا الحساب',
                    style: TextStyle(color: Colors.white54)),
              );
            }
            ...
            return ListView.builder(
              ...
              itemBuilder: (ctx, idx) {
                final adItem = AdItem.fromMap(adsData[idx]);
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    onTap: () {
                      // فتح صفحة الإعلان الكاملة الرسمية وليس مجرد صورة!
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (c) => FullAdDetailsScreen(
                    ...
                    leading: (adItem.imageUrls.isNotEmpty &&
                            adItem.imageUrls.first.isNotEmpty)
                        ? ClipRRect(
                            borderRadius: BorderRadius.circular(8),
                            child: SizedBox(
                              width: 55,
                              height: 55,
                              child: AppSmartImage(
                                imageUrl: adItem.imageUrls.first,
                                fit: BoxFit.cover,
                              ),
                            ),
                          )
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        const SizedBox(height: 2),
                        Text(
                            '${adItem.priceUsd != null ? "\$${adItem.priceUsd!.toInt()}" : ""} • ${adItem.governorate}',
                            style: const TextStyle(
...
/// ==============================================================================
// 🌟 البانوراما السينمائية الفاخرة المحصنة (تدعم القسمين العلوي 1 والسفلي 2 بدون تعليق)
// ==============================================================================
class CinemaUltraLuxuryPanoramaWidget extends StatefulWidget {
  final int slot;
  final List<BannerItem> banners;
  final int intervalSeconds;

  const CinemaUltraLuxuryPanoramaWidget({
    Key? key,
    required this.slot,
    required this.banners,
  ...
  @override
  State<CinemaUltraLuxuryPanoramaWidget> createState() =>
      _CinemaUltraLuxuryPanoramaWidgetState();
}

class _CinemaUltraLuxuryPanoramaWidgetState
    extends State<CinemaUltraLuxuryPanoramaWidget>
    with SingleTickerProviderStateMixin {
  late PageController _pageController;
  int _currentPage = 0;
  Timer? _timer;
  ...
  void _startAutoSlide() {
    _timer?.cancel();
    if (widget.banners.length <= 1 || _isSheetOpen) return;

    final seconds = widget.intervalSeconds.clamp(2, 12);
  ...
  @override
  void didUpdateWidget(covariant CinemaUltraLuxuryPanoramaWidget oldWidget) {
  ...
  void _openDetailsAndBookingSheet(BannerItem b) {
    setState(() {
      _isSheetOpen = true;
      _timer?.cancel();
    });
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    b.description.isNotEmpty
                        ? b.description
                        : 'عرض خاص ومميز معتمد عبر منصة سوق سوريا الشامل.',
                    style: const TextStyle(
                      color: Colors.white70,
                      fontSize: 12.5,
                      height: 1.5,
                    ...
                    if (b.whatsapp.isNotEmpty || b.phone.isNotEmpty) ...[
                      ...
                      Expanded(
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () {
                            final target =
                                b.whatsapp.isNotEmpty ? b.whatsapp : b.phone;
                            final clean = PhoneHelper.formatForWhatsapp(target);
                ...
                Container(
                  ...
                  child: ListTile(
                    ...
                    title: const Text('تريد إعلانك في هذه البانوراما؟ 💎',
                        style: TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.bold,
                            fontSize: 12.5)),
                    subtitle: const Text(
                        'احجز باقتك (3 أيام بـ 5\$ • أسبوع بـ 7\$ • شهر بـ 15\$)',
                        style: TextStyle(color: Colors.white70, fontSize: 10)),
                    trailing: const Icon(Icons.arrow_forward_ios_rounded,
                    ...
                    onTap: () {
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                            builder: (_) =>
                                const DedicatedPanoramaBookingScreen()),
                      );
                    },
                  ),
                ),
  ...
  Widget build(BuildContext context) {
    ...
    final isSlot1 = widget.slot == 1;
    final primaryAccent =
        isSlot1 ? const Color(0xFFD4AF37) : const Color(0xFF0284C7);
      ...
      builder: (context, child) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: primaryAccent.withOpacity(glowOpacity),
                blurRadius: 14,
                spreadRadius: 1,
                offset: const Offset(0, 3),
              ),
              ...
              children: [
                PageView.builder(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return GestureDetector(
                      onTap: () => _openDetailsAndBookingSheet(b),
                      child: Stack(
                        fit: StackFit.expand,
                        children: [
                          AppSmartImage(
                          ...
                          Positioned(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                b.badgeText.isNotEmpty
                                    ? b.badgeText
                                    : (isSlot1 ? 'VIP ★' : 'عرض مميز'),
                                style: TextStyle(
                                  color: primaryAccent,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 10,
                      ...
                      children: widget.banners.asMap().entries.map((entry) {
                        ...
                        return Expanded(
                          child: Container(
                            ...
                            margin: const EdgeInsets.symmetric(horizontal: 2),
                            decoration: BoxDecoration(
                              color: isCurrent ? primaryAccent : Colors.white24,
                              borderRadius: BorderRadius.circular(2),
                            ),
                          ),
                        );
...
// ==============================================================================
// 💎 صفحة حجز البانوراما المستقلة مع باقاتها المخصصة (3 أيام بـ 5$، أسبوع بـ 7$، شهر بـ 15$)
// ==============================================================================
class DedicatedPanoramaBookingScreen extends StatefulWidget {
  const DedicatedPanoramaBookingScreen({Key? key}) : super(key: key);

  @override
  State<DedicatedPanoramaBookingScreen> createState() =>
      _DedicatedPanoramaBookingScreenState();
}

class _DedicatedPanoramaBookingScreenState
    extends State<DedicatedPanoramaBookingScreen> {
  final AppStateManager _manager = AppStateManager();

  final List<Map<String, dynamic>> _panoramaPlans = [
    {
      'days': 3,
      'priceUsd': 5.0,
      'title': 'عرض 3 أيام ⏱️',
      'desc': 'ظهور بارز أعلى الصفحة الرئيسية'
    },
    {
      'days': 7,
      'priceUsd': 7.0,
      'title': 'عرض أسبوع كامل (7 أيام) 🌟',
      'desc': 'الأكثر طلباً للشركات والمتاجر'
    },
    {
      'days': 30,
      'priceUsd': 15.0,
      'title': 'عرض شهر كامل (30 يوم) 👑',
      'desc': 'أفضل قيمة وتثبيت دائم بالصدارة'
    },
  ];
  ...
  void initState() {
    super.initState();
    _selectedPlan = _panoramaPlans[1];
  ...
  @override
  Widget build(BuildContext context) {
    final double priceUsd = (_selectedPlan['priceUsd'] as num).toDouble();
    final int priceSyp = (priceUsd * _manager.exchangeRateUsdToSyp).toInt();
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: const Color(0xFF0F172A),
          title: const Text('حجز إعلان في البانوراما الرئيسية 💎',
              style: TextStyle(
                  color: Colors.white,
                  fontSize: 14.5,
                  fontWeight: FontWeight.bold)),
        ...
        body: ListView(
          ...
          physics: const BouncingScrollPhysics(),
          children: [
            const Text('1. حدد باقة حجز البانوراما المناسبة لك:',
                style: TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                    fontSize: 13.5)),
            const SizedBox(height: 8),
            ..._panoramaPlans.map((plan) {
              final isSel = _selectedPlan['days'] == plan['days'];
              final planSyp =
                  (plan['priceUsd'] * _manager.exchangeRateUsdToSyp).toInt();
              ...
              return Card(
                ...
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(12),
                  side: BorderSide(
                      color: isSel ? const Color(0xFFD4AF37) : Colors.white10,
                      width: isSel ? 1.8 : 1),
                ),
                child: ListTile(
                  ...
                  leading: CircleAvatar(
                    backgroundColor:
                        isSel ? const Color(0xFFD4AF37) : Colors.white12,
                    child: Icon(Icons.photo_library_rounded,
                        color: isSel ? Colors.black : Colors.white70, size: 20),
                  ),
            ...
            const SizedBox(height: 16),
            const Text('2. بيانات إعلان البانوراما والتواصل:',
                style: TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                    fontSize: 13.5)),
            ...
            ElevatedButton.icon(
              ...
              icon: const Icon(Icons.add_photo_alternate,
                  color: Color(0xFFD4AF37)),
              label: Text(
                  'اختيار صور البانوراما (${_selectedImages.length} صور محددة)',
                  style: const TextStyle(fontSize: 12)),
              onPressed: () async {
                final picker = ImagePicker();
                final picked = await picker.pickMultiImage(
            ...
            const SizedBox(height: 16),
            const Text('3. سداد الرسوم وإرفاق إشعار الدفع:',
                style: TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                    fontSize: 13.5)),
            ...
            TextField(
              ...
              style: const TextStyle(color: Colors.white, fontSize: 13),
              decoration: const InputDecoration(
                labelText: 'رقم المعاملة أو الإشعار (اختياري)',
                labelStyle: TextStyle(color: Colors.white54, fontSize: 11),
                filled: true,
                fillColor: Color(0xFF1E293B),
                border: OutlineInputBorder(),
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(Icons.receipt_long_rounded,
                                color: Color(0xFFD4AF37), size: 20),
                            SizedBox(width: 6),
                            Text('اضغط لإرفاق صورة إشعار أو إيصال التحويل 📸',
                                style: TextStyle(
                                    color: Colors.white70, fontSize: 12)),
                          ],
                        ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                    ...
                    : () async {
                        if (_titleCtrl.text.trim().isEmpty ||
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text(
                                    'يرجى كتابة عنوان البانوراما ورقم الهاتف')),
                          );
                          return;
                        }
                        if (_selectedImages.isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text(
                                    '⚠️ يرجى اختيار صورة واحدة على الأقل للبانوراما')),
                          );
                          return;
                        }
                        if (_receiptBytes == null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                                content: Text(
                                    '⚠️ يرجى إرفاق صورة إيصال التحويل لإثبات الدفع')),
                          );
                          return;
                        }
                        ...
                        try {
                          bannerUrls = await StorageUploadService
                              .uploadMultipleImageBytes(
                            bucketName: kStorageBucketBanners,
                        ...
                        await _manager.submitPaymentAuditRequest(
                          planId: 'panorama_${_selectedPlan['days']}',
                          planName: 'حجز بانوراما: ${_selectedPlan['title']}',
                          priceUsd: priceUsd,
                          gateway: _selectedGateway,
                          refOrTxId: _refCtrl.text.trim().isNotEmpty
                              ? _refCtrl.text.trim()
                              : 'مرفق صورة الإيصال',
                          userName: _manager.currentUserName,
                          ...
                          requestType: 'panorama_booking',
                          durationLabel: _selectedPlan['title'],
                          durationHours: (_selectedPlan['days'] as int) * 24,
                          receiptBytes: _receiptBytes,
                          bannerImages: bannerUrls,
                          bannerTitle: _titleCtrl.text.trim(),
                          bannerSubtitle: _subtitleCtrl.text.trim(),
                          bannerLinkUrl: '',
                        );
                            ...
                            builder: (ctx) => AlertDialog(
                              ...
                              title: const Row(
                                children: [
                                  ...
                                  SizedBox(width: 8),
                                  Text('تم إرسال طلب حجز البانوراما 🚀',
                                      style: TextStyle(
                                          color: Colors.white, fontSize: 14)),
                                ],
                              ),
                              content: const Text(
                                'تم استلام صور البانوراما وإشعار الدفع بنجاح.\nسيتم مراجعة الطلب وتفعيل إعلانك فوراً في أعلى الصفحة الرئيسية!',
                                style: TextStyle(
                                    color: Colors.white70, fontSize: 12),
                              ),
                              actions: [
                ...
                child: _isSubmitting
                    ? const CircularProgressIndicator(color: Colors.black)
                    : const Text('إرسال طلب حجز البانوراما والإيصال للإدارة 🚀',
                        style: TextStyle(
